...
% inspect data
% ECG = ECGIN;
ECG=zeromean(baselinecor(subtracthum(ECGIN))); % oostep1
ECG(ECG>2)=2;
ECG(ECG<-2)=-2;
...
sc=.5*max(rrms(SPECS.onsetqrs:SPECS.endtwave))/max(abs(ddrrms(SPECS.onsetqrs-2:SPECS.endtwave-2)));
...
tdom = baselinecor(tdom,find(tdom== min(tdom(1:200))),length(tdom))';
