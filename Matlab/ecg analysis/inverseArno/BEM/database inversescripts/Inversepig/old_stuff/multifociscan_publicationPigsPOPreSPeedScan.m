function [bestfoci,bestdep,outp] = multifociscan_publicationPigs(varargin)
...
SCAN.usetime = 60;
SCAN.regionR = 30;
SCAN.lpass   = 0; % 0 voor hakkel, 5 normaal
SCAN.scanmode= 1;
SCAN.useleads=[];
useFocus = -1;
showplots=true;
...
SCAN.PSIREF = baselinecor(lowpassma(GEOM.BSM(:,GEOM.SPECS.onsetqrs:GEOM.SPECS.endtwave),5)); % fixed filter, lpass for removing spikes in FW signals
...
while  nrClust < SCAN.clusters % oostep1 20131211
    ...
    if SCAN.lpass<2
        PSIA =SCAN.AMA * getSmode(ones(length(GEOM.VER),1) * ( 1 : SCAN.qrsduration ),dep,SCAN.rep, GEOM.SPECS,SCAN.scanmode,GEOM);
    else
    ...
    if size(PSIA,1) == size(GEOM.LAY,1)-1
        ...
        if showplots
            ...
            sigplot(PSIANormal,'',GEOM.LAY,1/maxAmpl,'k',1,0);
%             pause
        end
    end
    if showplots
...
for inode=scanrange %1:length(GEOM.VER)
    ...
    if isempty(initdep)
        ...
        ishift=1;
        for shiftl=shiftvec
            dep = min([shiftl + SCAN.DIST2W(:,inode) / SCAN.maxVelocity, initdep],[],2); % use maxv from 1st focus instead??
...
if SCAN.anisotropyRatio < 100 %1
    dep = GEOM.DIST25(:,inode) / SCAN.initialVelocity;
    depw = SCAN.DIST2W(:,inode) / SCAN.initialVelocity;
    
    
    if SCAN.initialActTime >dt0max
        warning('Limited initial activation time to %0.0f ms',dt0max);
    end
    %         dt0= min(60,SCAN.initialActTime);
    
    depAbove = find(dep >  dt0 );
    maxv=min(SCAN.maxVelocity,(max(depw) - dt0) * SCAN.initialVelocity/ (SCAN.qrsduration - dt0) );
    %        maxv=SCAN.maxVelocity; % fixed V2
    dep(depAbove) = dt0 + (depw(depAbove) - dt0) * SCAN.initialVelocity / maxv;
else
    dep = SCAN.DIST2W(:,inode) / SCAN.initialVelocity;
    depAbove = find(dep >  dt0 );
    maxv=min(SCAN.maxVelocity,(max(dep) - dt0) * SCAN.initialVelocity/ (SCAN.qrsduration - dt0) );
    %        maxv=SCAN.maxVelocity; % fixed V2
    dep(depAbove) = dt0 + (dep(depAbove) - dt0) * SCAN.initialVelocity / maxv;
end
bestdep = dep;
maxvelo=maxv;
...
if cor > 0
    for dt=0:4:SCAN.initialActTime %-16 %2:2:SCAN.initialActTime
        dt0 = min(SCAN.initialActTime, dt0max) -dt;  % intialActTime relative to first activation
        % the propagtion velocity is assumed to be homogenoeus initially. Probably
        % the numerical accuracy of the method used is not high enough to tell the
        % difference
        if SCAN.anisotropyRatio < 100% 1 % Pig?, different anisotropy for initial part
            dep = GEOM.DIST25(:,inode) / SCAN.initialVelocity ;
            depw = SCAN.DIST2W(:,inode) / SCAN.initialVelocity ;
            
            %                 dt0= min(60,SCAN.initialActTime);
            depAbove = find(dep >  dt0 );
            maxv=min(SCAN.maxVelocity,(max(depw) - dt0) * SCAN.initialVelocity/ (SCAN.qrsduration - dt0-dt) );
            %                maxv=SCAN.maxVelocity; % fixed V2
            dep(depAbove) = dt0 + (depw(depAbove) - dt0) * SCAN.initialVelocity / maxv;
            dep=dep+dt;
        else
            dep = SCAN.DIST2W(:,inode) / SCAN.initialVelocity;
            depAbove = find(dep >  dt0 );
            maxv=min(SCAN.maxVelocity,(max(dep) - dt0) * SCAN.initialVelocity/ (SCAN.qrsduration - dt0-dt) );
            %        maxv=SCAN.maxVelocity; % fixed V2
            dep(depAbove) = dt0 + (dep(depAbove) - dt0) * SCAN.initialVelocity / maxv;
            dep=dep+dt;
