...
while  nrClust < SCAN.clusters
    ...
    PSIA =lowpassma(SCAN.AMA * getSmode(ones(length(GEOM.VER),1) * ( 1 : SCAN.qrsduration ),dep,SCAN.rep,GEOM.pS,[],SCAN.scanmode),SCAN.lpass);
...
for inode=1:length(GEOM.VER)
    ...
    if isempty(initdep)
        dep = calcDepolarization(SCAN, inode,GEOM);       
    else
    ...
    PSIA =lowpassma(SCAN.AMA*getSmode(ones(length(GEOM.VER),1) * (1:maxt),dep,rep,GEOM.pS,[],SCAN.scanmode),SCAN.lpass);
...
%     rdsinit(inode) = norm(SCAN.PSIREF(:,1:min(maxt,SCAN.initialActTime)) - PSIA(:,1:min(maxt,SCAN.initialActTime)),'fro')/norm(SCAN.PSIREF(:,1:min(maxt,SCAN.initialActTime)),'fro');
    ...
    if SCAN.usecor && cors(inode) < 0
        cors(GEOM.ADJsurf(inode,:) > 0)= cors(inode);
    elseif ~SCAN.usecor && rds(inode) > 1.5
        rds(GEOM.ADJsurf(inode,:) > 0)= rds(inode);
    end
...
%%========================================================================
function dep = calcDepolarization(SCAN,inode,GEOM)
...
% qtriplot(GEOM.VER,GEOM.ITRI,'depinit');
% qtriplot(dep);
% qtriplot('panel 2,1');
% depBelow=find(dep<=SCAN.initialActTime);
...
% qtriplot(GEOM.VER,GEOM.ITRI,'depnew');
% qtriplot(dep);
% qtriplot(GEOM.VER(depBelow,:),[],'depBelow');
