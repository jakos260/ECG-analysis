...
leadset='all';
group=[];
sub=[ 9 10 7 8]; % can be an array now
...
doinv=true % perform inverse after intial estimate
interpol=0; % if 1 removed leads are interpolated, no interpolation for bucket
...
%initialtype
initmode=0 % methods for intial estimate
...
% DATE='20141031' % overrule when continuing earlier analysis
...
elseif sub == 7
    ...
    dirout= ['/Users/peteroosterhoff/Documents/Werk/STW/Data/results_b/' subject '/'];
elseif sub == 8
    subject='Pig08';
...
for ibeat = 1 : length(bsmfiles)
    ...
    for wrd=0%wrd=wrd
        if wrd
            ...
            MUDEP=[1e-6,5e-7];%[5e-5,1e-5,5e-6];%5e-6;
            murep=5e-5;
        end
...
%                                         warning('Bucket01 fixed focus');
                            ...
                            if ~doinv
                                ...
                                if showplots
                                    ...
                                    AltPSIAinitnohakkel =lowpassma(GEOM.AMA*getSmode(T,altdep,altrep,GEOM.SPECS,1, GEOM),lpass);

                                    
                                    PSIA =lowpassma(GEOM.AMA*getSmode(T,meas.depfinal,meas.repfinal,GEOM.SPECS,1,GEOM),lpass);
...
%                                     initdep(initdep<min(initdep(GEOM.typ==1)))=min(initdep(GEOM.typ==1)); % nothing before epi
                                    ...
                                    PSIAinitnohakkel =lowpassma(GEOM.AMA*getSmode(T,measinit.dep,measinit.rep,GEOM.SPECS,1, GEOM),lpass);
                                    PSIAinit = lowpassma(GEOM.AMA*getSmode(T,measinit.dep,measinit.rep,GEOM.SPECS,1),lpass);
