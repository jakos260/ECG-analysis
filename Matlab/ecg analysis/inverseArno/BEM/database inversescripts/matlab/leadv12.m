...
if do9
	...
	for k=1:3
		...
		for i=nmap:-1:1 
			...
			if ~bw
				if k==1
					plot(sampT*(0:length(lead1)-1), lead1,cols(i,:),'Linewidth',linew);
				elseif k==2
					plot(sampT*(0:length(lead2)-1), lead2,cols(i,:),'Linewidth',linew);
				else
					plot(sampT*(0:length(lead3)-1), lead3,cols(i,:),'Linewidth',linew);				
				end
			else
				if k==1
