...
%         DefAns{3}=varargin{3};
    for i=1:3
        ...
        elseif isempty(varargin{i})
            ...
            if i==1
                [dumx , DefAns{i}]=index2blocks(varargin{i},0); % no preprocessing to preserve order
            else
...
if nouiflag
    ...
    prompt{1}=sprintf('Select channels (matlab format without the []: 1:5 14)Empty reads all. [%s]',DefAns{1});
...
% Breakpoint here to create 'degoedeheader.mat'
