...
if doauto
    ...
    for i=2:length(qrss)
        if qrss(i) - qrss(i-1)+ignoredint > 400 % minimal interval between qrss
    ...
    if (length(DATA.BEATS) < size(DATA.BSM,2)/2000 || max(diff(DATA.BEATS))>6000) && size(BSMfiltextra,2) > 2000
        ...
        ignoredint=0;
        for i=2:length(qrss)
            if qrss(i) - qrss(i-1) +ignoredint > 300 % minimal interval between qrss
                DATA.BEATS = [DATA.BEATS; qrss(i) - DATA.PQint];
                ignoredint=0;
            else
                ignoredint=ignoredint+qrss(i);
            end
        end
    end
end
...
global OPTIONS
global D mlas
[OPTIONS.FileName,OPTIONS.PathName,FilterIndex] = uigetfile({'*.mat','*.*'},'Select data file.','/Users/peteroosterhoff/Documents/Werk/STW/Data/Pig10/BioSemi/Export');
