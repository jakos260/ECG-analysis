...
%
% qtriplot('somestring %f',var1,...). If more than 1 argiments are passed and first argument is a string , 
% arguments will be passed to sprintf and the result processed as a qtriplot command. 
% First argument must be a string with sprintf formatting (e.g. %0.1f, %s).
%
% Qtriplot(C)
% Process cell array C as qtriplot() commands.
% If C is 1D cell array of 1D cell arrays (preferred), loops over outer array.
% If C is a 2D cell array rows are passed to qtriplot(), removing empty cells.
% Example: 
%   GeoName='heart';
%   qtriplot({{VER,ITRI,GeoName}
%               {'color %s=blue',GeoName}});
%
% Recording qtriplot() arguments:
% qtriplot('RECORD'): start recording arguments for all subsequent calls to
% qtriplot.m in a command buffer. First wipes the command buffer.
% 
% qtriplot('SAVE', filepath): Saves command buffer to file 'filepath', 
% Stops recording and wipes command buffer.
% 
% Replay saved commands with:
...
%
% The program qtriplot is not in the PATH environment variable in order to
% can be started. As an alternative, you can define the global variable
% qtriplot_path to point to the location of the program.
...
global qtriplot_con;
persistent RECORD % if true commands are stored in qtriplotcmds
persistent qtriplotcmds
if isempty(RECORD)
    RECORD=false;
end
% pause(0.1)
...
%         fprintf('%d: %s\n',i,varargin{i});
%     else
%         fprintf('%d:[%dx%dx%d]\n',i,size(varargin{i},1),size(varargin{i},2),size(varargin{i},3));
%     end
% end



% Process record/save commands
if nargin>0 && all(strcmp(varargin{1},'RECORD'))
    RECORD=true;
    qtriplotcmds={};
    return % never record a record command
end

if nargin>0 && all(strcmp(varargin{1},'SAVE'));
    if isempty(qtriplotcmds)
        warning('Nothing to save');
        return
    end
    if nargin==1 || isempty(varargin{2})
        [Filename, Pathname]=uiputfile('*.mat','Select file name for qtriplot save file');
        filepath=fullfile(Pathname, Filename);
    else
        filepath=varargin{2};
    end
    save(filepath,'qtriplotcmds');
    RECORD=false;
    qtriplotcmds={};
    return; % Do not record save command
end

% Process multiple commands in cell array 
if nargin>1 && ischar(varargin{1})
    % Command in sprintf format
    qtriplot(sprintf(varargin{:}));
    return
elseif nargin==1 && iscell(varargin{1})
    % Multiple commands in cell array
    C=varargin{1};
    if numel(C)==length(C) && iscell(C{1})
        % 1D cell array of cell(arrays)
        for i=1:length(C)
            qtriplot(C{i}{:});
%             pause(0.1);
        end
        return
    else % 2D array of cells, process per row, remove empty padding
        for i=1:size(C,1)
            l=size(C,2);
            while isempty(C{i,l}) && l>0
                l=l-1;
            end
            if l>0
                qtriplot(C{i,1:l});
...
if (con<0)
  ...
  qtriplot_path = 'C:\Users\Jeanne\Documents\qtriplot';
