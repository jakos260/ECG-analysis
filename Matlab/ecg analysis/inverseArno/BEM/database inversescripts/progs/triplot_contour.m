...
%                         intervals
%                zebra will be overruled by% stripes=[desired levels]
...
if ~exist('boxsize'), boxsize=max(norm3d(VER)) ; end
if ~exist('contourcolor'), contourcolor='white'; end
nosw=0;
funsw=0;
fun=zeros(nver,1);
ndat=1;
...
if ~exist('node')   node=min(unique(ITRI)); end
if ~exist('column') column=1;end
if ~exist('symcolor'), symcolor=0; ,end % symcolor==1 produces colorbar symmetric around zero
if ~exist('lsw'),lsw=1; end
if  column > maxcoll, column=maxcoll; end
if  column < 1, column=1; end

if exist('VALS') && ~isempty(VALS),
...
if exist('cmap')==0, cmap='avojet.mcm'; cmapnow=cmap; end
...
% plot contour lines
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if exist('VALS'), plotcontourlines, end
