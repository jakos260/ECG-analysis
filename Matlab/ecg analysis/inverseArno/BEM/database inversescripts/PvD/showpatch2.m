...
if length(varargin)<3
	...
	maxdep=max([dep; depref]);
	mindep=min([ dep; depref]);
	lines=[];posi=[];myview=[];
	pp=7;
	isolines=0;
	sublines=10;
...
if ~isempty(dep)
	...
	patch('Faces',ITRI,'Vertices',VER,...
		  'FaceLighting','phong','BackFaceLighting','reverselit','AmbientStrength',0.7,...
		  'FaceColor','r',...
		  'edgecolor',edges,'FaceAlpha',.5,'buttondownFcn','selectnode');  
end

if  ~isempty(depref)
	patch('Faces',ITRI_REF,'Vertices',VER_REF,...
		  'FaceLighting','phong','BackFaceLighting','reverselit','AmbientStrength',0.7,...
		  'FaceVertexCData',depref,'FaceColor','interp',...
		  'edgecolor',edges,'FaceAlpha',0.5,'buttondownFcn','selectnode');
    ...
    if ~isempty(ITRI_REF)
        ...
        hold on;
        plot3(VER_REF(:,1),VER_REF(:,2),VER_REF(:,3),'k.','markersize',5);
    end
