...
if mod(length(varargin)-3,2) ~= 0
	error('Input: VER, ITRI,orgVER, VER1,ITRI1, .....VERn,ITRIn');
elseif nargout > 1
	error('outputs: newVER');
else
	VER=varargin{1};
	ITRI=varargin{2};
	orgVER=varargin{3};
	...
	for i=2:length(varargin)/2
		...
		eval(['adj=tri2graph(ITRI_' num2str(i-1) ');']);
		adj=triu(adj);
		[a,b]=find(adj>0);
		eval(['edges_' num2str(i-1) '=[a b];']);
	end
end


for i=1:length(VER)
	[a,pi]=ismember(VER(i,:),VER);
	if sum(a) >3
		VER(pi,1)=orgVER(pi,1);
		disp('Two nodes coincided!!!!\n');
	end
end



adj=tri2graph(ITRI);adj=triu(adj);[a,b]=find(adj>0);
edges=[a b];
ner=0;err=1;
while err>0 & ner<=5
	...
	for edg=1:length(edges)
		...
		for i=1:ngeoms
			...
			if ~isempty(inter)
				...
				eval(['[a,f]=ismember(VER_' id '(tri,:),orgVER);']);
				f=f(:,1);f=f(f>0);
...
if err
	error(['fatal error!!!!  After ' num2str(ner) ' runs, still crossections found.'])
end
varargout{1}=VER;
...
for i=1:length(VER)
	...
	if length(a) >1
		error(['fatal error, ' num2str(length(a)) ' vertices with the same coordinates as VER ' num2str(i) ' !!']);
	end
	if ~isempty(a)
		otherVER(a,:)=VER(i,:);
	end
