...
% display('oostep1: force 1791 as first focus');
% beatfoci=1791;
%  dep=GEOM.DIST2W(:,bestfci)/SCAN.MAX_MYO_VS;
% nrClust=1;
...
while  nrClust < clusters
    ...
    if size(PSIA,1) == size(GEOM.LAY,1)-1
        ...
        fh=figure(100);clf;set(fh,'Name','Init:BSM QRS'); sigplot(SCAN.PSIREF(:,1:size(PSIA,2)),'',GEOM.LAY,1.3/maxAmpl,'b',1,0);
        hold on;
        sigplot(PSIA,'',GEOM.LAY,1.3/maxAmpl,'r',1,0);
    end
    fh=figure(101);set(fh,'Name','Init:Activation time');showAtria(GEOM.VER,GEOM.ITRI,bestdep,'nodes',bestfoci,'onodes',bestsinks,'range',[0 SCAN.qrsduration]);drawnow
end
...
%     display('HACK:ony consider Pig09_refined Endo node 1791 and/or epi node 1734');
    ...
    %     % hack oostep1
%         if inode==1
%             display('limiting signal to intial part');
%         end
%         PSIA=PSIA(:,1:20);
