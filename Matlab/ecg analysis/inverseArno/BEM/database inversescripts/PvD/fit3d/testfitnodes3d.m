...
% X=loadasci('defHVER.hver');
% X=X+ones(size(Yref,1),1)*(mean(Yref)-mean(X));
...
while loop1>=1,
   ...
   while break1==0,
       lambda=2*lambda;
       if lambda > 1.e+10*gtgnorm, break1=1; disp(num2str(lambda)),   bestnorm=testnorm;break, end
