...
if do9
	...
	for k=1:3
		...
		for i=nmap:-1:1 
			...
			if ~bw
				if k==1
					plot(sampT*(0:length(phi2)-1), phi2-phi1,cols(i,:),'Linewidth',linew);
				elseif k==2
					plot(sampT*(0:length(phi3)-1), phi3-phi1,cols(i,:),'Linewidth',linew);
				else
					plot(sampT*(0:length(phi2)-1), phi3-phi2,cols(i,:),'Linewidth',linew);				
				end
			else
				if k==1
					plot(sampT*(0:length(phi2)-1), phi2-phi1,styles(icyc(i,length(styles)),:),'Linewidth',linew);
				elseif k==2
					plot(sampT*(0:length(phi3)-1), phi3-phi1,styles(icyc(i,length(styles)),:),'Linewidth',linew);
				else
					plot(sampT*(0:length(phi2)-1), phi3-phi2,styles(icyc(i,length(styles)),:),'Linewidth',linew);				
				end
			end
		end
...
for k=1:9
	...
	for i=nmap:-1:1
		...
		if k==6 %V3
			if ~bw
				plot(sampT*(0:length(PHI(V,:))-1), mean(PHI(V-1:V,:)),cols(i,:),'Linewidth',linew);
			else
		...
		elseif k<=3
			if ~bw
				plot(sampT*(0:length(phi)-1), phi,cols(i,:),'Linewidth',linew);
			else
			...
			if ~bw
				plot(sampT*(0:length(PHI(V,:))-1), PHI(V,:),cols(i,:),'Linewidth',linew);
			else
