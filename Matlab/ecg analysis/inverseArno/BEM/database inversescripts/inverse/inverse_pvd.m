...
TST.wrd = norm(bsxfun(@times,TST.RES,scale),'fro') ./ INV.normphi;
...
function S=getS(T,dep,rep,SPECS,notchpot,scaleAmpl,mode,GEOM)

if mode == 4
    SPECS.scaleAmpl = scaleAmpl;
    if size(T,2) < max(rep) + 50
        Tt=ones(length(dep),1)*(0:max(rep) + 50);
        S=getSmode(Tt,dep,rep,SPECS,4,GEOM);
        S=S(:,1:size(T,2));
    else
        S=getSmode(T,dep,rep,SPECS,4,GEOM);
    end
else
    SPECS.scaleAmpl = notchpot;
    S=getSmode(T,dep,rep,SPECS,mode,GEOM);
end