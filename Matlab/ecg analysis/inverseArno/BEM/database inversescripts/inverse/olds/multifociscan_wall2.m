function [bestfoci,bestdep,outp]=multifociscan_wall2(varargin)
...
% if SCAN.qrsduration > 130
%     SCAN.MAX_MYO_VS = SCAN.MAX_MYO_VS - 0.1; % 0.7
% end
% if SCAN.qrsduration > 180
%     SCAN.MAX_MYO_VS = SCAN.MAX_MYO_VS - 0.1; % 0.7
% end
...
%     if size(PSIA,1) > 10
%         qtriplot('delete initialbsm')
%         qtriplot(GEOM.VER,GEOM.ITRI,'initialbsm');
%         qtriplot(bestdep-min(bestdep));
%         qtriplot('step 10');
...
%         qtriplot('panel 1,1');    
%     else
%         qtriplot('delete initialv12')
%         qtriplot(GEOM.VER,GEOM.ITRI,'initialv12');
%         qtriplot(bestdep-min(bestdep));
%         qtriplot('step 10');
%         qtriplot('horizontal 2');
%         qtriplot('vertical 2');
...
%% scan for foci -----------------------------------------------
peakRms = find(rms(SCAN.PSIREF(:,1:SCAN.qrsduration)) == max(rms(SCAN.PSIREF(:,1:SCAN.qrsduration))));
...
for inode=1:length(GEOM.VER)
    ...
    if ~isempty(initdep) && SCAN.isSinus && ~GEOM.purkinjever(inode)
        continue
    end
    ...
    rrms = rms(PSIA);
    peakrRms = find(rrms == max(rrms));
    dep = dep + (peakRms - peakrRms)/2;
    PSIA =lowpassma(SCAN.AMA*getSmode(ones(length(GEOM.VER),1) * (1:max(SCAN.qrsduration, round(max(dep)) )),dep,SCAN.rep,GEOM.pS,[],SCAN.scanmode),SCAN.lpass);
    COR=corrcoef(PSIA,SCAN.PSIREF(:,1:size(PSIA,2)));
    cors(inode) = COR(2,1);
    rds(inode) = norm(SCAN.PSIREF(:,1:size(PSIA,2)) - PSIA,'fro')/SCAN.normphi;
    deps(:,inode) = dep;
end
...
%             end
            ...
            elseif cor1 >= cor2
                cors(inode) = cor1;
                if isempty(initdep) && min(rds) >= 0.7 
                    deps(:,inode) = dep1;
                end
            else
                cors(inode) = cor2;
...
% disp( ['std ' num2str([std(cors) std(rds)] )])
...
% if std(cors) < 0.08
%     A(GEOM.purkinjever == 0,2) = A(GEOM.purkinjever == 0,2) -0.1;
%     A(GEOM.purkinjever == 0,3) = A(GEOM.purkinjever == 0,3) +0.05;
% end

% if isfield(GEOM,'typ') && ~isempty(initdep)
%     baseD =25;
%     for i=1:length(GEOM.DIST)
%         if min(GEOM.DIST(i,GEOM.typ>3)) < baseD
%             A(i,2) = A(i,2) -0.1;
%             A(i,3) = A(i,3) +0.05;     
%         end
%     end
...
if isempty(initdep)  || ~SCAN.usecor %|| ( isempty(initdep) && min(rds) < 0.7 )  
    A(A(:,3)>=10,:)=[];  
    A=sortrows(A,3);
else
    A(A(:,2)< 0,:)=[];
...
if ~isempty(A)
%     if ~isempty(initdep) 
%         focusfound = sum(deps(:,A(1)) == initdep) < 0.9 * length(initdep);
%     else
        focusfound  = 1;
%     end    
