function [bestfoci,bestdep,outp]=multifociscan_v12a(varargin)
...
SCAN.PSIREF = lowpassma(GEOM.BSM(:,GEOM.specs(2):GEOM.specs(5)),1);
...
rrms =rms(lowpassma(SCAN.PSIREF(:,1:GEOM.specs(3)-GEOM.specs(2)+1),10));
drrms=diffrows(rrms);
SCAN.usetime = 30;
for i=2:45
    if abs(drrms(i-1)) < 0.008
        SCAN.usetime = i;
    end
end
...
ECGINIT = baselinecor(lowpassma(SCAN.PSIREF(:,1:GEOM.specs(3)-GEOM.specs(2)),10));
corMode = 1;
...
%         continue;
    ...
    if isinf(SCAN.MAX_MYO_VS)
        ...
        dep1Above = find(dep >= SCAN.usetime);
        dep1(dep1Above) = SCAN.usetime + (dep1(dep1Above)-SCAN.usetime) * ((SCAN.qrsduration - SCAN.usetime) / (max(dep1)- SCAN.usetime) );
        dep1 = dep1 * (SCAN.qrsduration/ max(dep1) );
    end
    
    if isinf(SCAN.usetime)
        maxt = max(SCAN.qrsduration, round(max(dep)));
        PSIA =lowpassma(SCAN.AMA*getSmode(ones(length(GEOM.VER),1) * (1:maxt),dep,SCAN.rep,GEOM.pS,[],SCAN.scanmode),SCAN.lpass);
        COR=corrcoef(PSIA,SCAN.PSIREF(:,1:size(PSIA,2)));
        cors(inode) = COR(2,1);
        ...
        maxt = max(SCAN.qrsduration, round(max(dep)));
        PSIA =lowpassma(SCAN.AMA*getSmode(ones(length(GEOM.VER),1) * (1:maxt),dep,SCAN.rep,GEOM.pS,[],SCAN.scanmode),SCAN.lpass);
        maxt1 = max(SCAN.qrsduration, round(max(dep1)));
        PSIA1 =lowpassma(SCAN.AMA*getSmode(ones(length(GEOM.VER),1) * (1:maxt1),dep1,SCAN.rep,GEOM.pS,[],SCAN.scanmode),SCAN.lpass);

        corsinit(inode) = max(compCor(PSIA(:,1:min(maxt,SCAN.usetime)),SCAN.PSIREFINIT(:,1:min(maxt,SCAN.usetime)),corMode),...
                              compCor(PSIA1(:,1:min(maxt1,SCAN.usetime)),SCAN.PSIREFINIT(:,1:min(maxt1,SCAN.usetime)),corMode) );
        corsterm(inode) = max(compCor(PSIA(:,SCAN.termtime:maxt),SCAN.PSIREF(:,SCAN.termtime:maxt),corMode),...
                              compCor(PSIA1(:,SCAN.termtime:maxt1),SCAN.PSIREF(:,SCAN.termtime:maxt1),corMode) );
        cors(inode) =  max(compCor(SCAN.PSIREF(:,1:size(PSIA,2)),PSIA,corMode),...
                            compCor(SCAN.PSIREF(:,1:size(PSIA1,2)),PSIA1,corMode) );
...
%         rdsinit(inode) = norm(SCAN.PSIREF(:,1:min(maxt,SCAN.usetime)) - PSIA(:,1:min(maxt,SCAN.usetime)),'fro')/norm(SCAN.PSIREF(:,1:min(maxt,SCAN.usetime)),'fro');
        rds(inode) = min(norm(SCAN.PSIREF(:,1:maxt) - PSIA(:,1:maxt),'fro')/norm(SCAN.PSIREF(:,1:maxt),'fro'),...
                         norm(SCAN.PSIREF(:,1:maxt1) - PSIA1(:,1:maxt1),'fro')/norm(SCAN.PSIREF(:,1:maxt1),'fro'));
    end
    if inode==1 || inode ==find(GEOM.ADJ(1,:) == min(GEOM.ADJ(1,GEOM.ADJ(1,:)>0 & GEOM.ADJsurf(1,:)==0)))
        if inode==1
            PSIA1=PSIA;
            dep1=dep;
        else
...
% if size(PSIA,2) - SCAN.termtime < 10
%     corsterm = cors;
% end
% if SCAN.usetime <= 15
%     corsinit = cors;
...
% d=-1;
% while sum(d==-1)/length(d) >0.95
%     d = corsinit;
%     d(cors     < deltaCor ) = -1;
%     d(corsinit < deltaCor ) = -1;
%     d(corsterm < deltaCor ) = -1;
...
while isempty(ITRI)
    while isempty(ITRI)
        ...
        for i=1:length(GEOM.ITRI)
            if  any(corsinit(GEOM.ITRI(i,:)) > 0 ) &&...max(corsinit) - deltaCor) &&...
