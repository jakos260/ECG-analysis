...
int pntontotri(double rin[3], double rpmin[3],int npnt, int ndhk, double (*pnt)[3], int (*dhk)[3])
{
	int	idhk, k, mindhk=0;
...
void print_help(void)
 {
  mexPrintf("");
  mexPrintf("  usage: pnttridist [dist,loc,onItri]=pnttridist(VER,ITRI,PNTS)\n");
  mexPrintf("");
             		...
             		int nrhs, const mxArray *prhs[] )
{
  /* Declare variables. */  
	...
	double (*pnt)[3], (*pnt1)[3];
    double pnt2[3];
	double *ver, *ver1, *DIST, *LOC, *itri, *VER;
	...
	for (i=0; i<npnt1; i++)
	{
		...
		minIdhk=pntontotri(pnt1[i],pnt2, npnt, ndhk, pnt, dhk);
