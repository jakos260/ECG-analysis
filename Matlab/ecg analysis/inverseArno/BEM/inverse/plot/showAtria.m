...
if length(varargin) < 3,
    ...
    while pp <= length(varargin),
        ...
        elseif length(varargin{pp}) == 1,
            ...
            if length(varargin) > pp, onode = varargin{pp}; pp = pp + 1; end
...
if isempty(lines),
    if sym == 0,
        ...
        lines = loadmat('mymap_basic.mcm');
        lines = lines(end:-1:1,:);
    end
end

if abs(mindep) > 10  && ~exist('givenMini','var'), mini = min(10*floor(mindep/10)); end

if sym && ~exist('givenMini','var'), mini = -max(abs(dep)); maxi = max(abs(dep));   end

if mini == maxi, maxi = mini + 1; end

colormap(lines);

if isempty(posi), clf; pos = get(gcf,'Position'); posi = [0 0 1 1]; end

if posi(3) > .5,
    axes('Position',[posi(1)+posi(3)*0.42 posi(2)+posi(4)*0.2 0.1*posi(3) 0.6*posi(4)]);
    axis off
    caxis([mini maxi])
    colorbar;
end
...
if ~isempty(endoVER), DrawLines(VER,ITRI,endoVER); end
...
for i = 1:length(adj),
    ...
    if endo ~= epi, adj(i,:) = 0; adj(:,i) = 0; end
