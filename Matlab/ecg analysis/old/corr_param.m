...
p(4) = -0.05;
p(5) = 0;
S_nodes= gets(T,dep,rep,p,4);    % mode = 4 
lp=5;
doplot = 0;
...
%% Parameter sweep p1, p3, p4 for the best vertex of each channel

p1_grid = 0.1:0.2:2;               % upstroke slope
p3_grid = -0.050:0.010:0.010;      % plateau slope
p4_grid = -0.050:0.010:0.010;      % repolarization slope
...
for c = 1:n_CH
    printProgress(c, n_CH, sprintf("channel %s", chan_names{1, c}));
    ...
    for p1 = p1_grid
        for p3 = p3_grid
            for p4 = p4_grid
                p = [p1, p2_fix, p3, p4, p5_fix];
                S_nodes_sweep = gets(T, dep, rep, p, 4);
...
ParamTable = table(chan_idx(:),string(chan_names(:)), BestVX(:), [BestP1(:) BestP3(:) BestP4(:)],  BestR(:), BestR_tuned(:), diff_AMP(:),RMSE(:),dAUC(:),RMSE_QRS(:),'VariableNames', {'ChanIdx','ChanName','BestVertex','BestP1,BestP3,BestP4','BestR_old','BestR_new','Amplitude difference','rmse(:)','dAUC(:)','rmse_QRS(:)'} );
disp(ParamTable);
