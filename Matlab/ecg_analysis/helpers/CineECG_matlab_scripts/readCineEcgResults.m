...
if length(varargin) < 1
    ...
    pp = 2;
    while pp <= nargin
        if isstring(varargin{pp})
            varargin{pp} = char(varargin{pp});
        end
        ...
        if ischar(varargin{pp})
            ...
            switch key
                ...
                case 'doall'
                    ...
                    pp = pp + 1;
                otherwise
                    error(['unknown parameter: ', varargin{pp}]);
            end
        else
            error('expected string parameter at %d position, but %s recevied.', pp, class(varargin{pp}));
        end
    end
end
...
iEcgv=0;
iEcga=0;
totalIter = length(CASEDATA);
for i=1:totalIter   
    plikInfo = ['File: ', CASEDATA{i}.ECGfilename.Text];
    printProgress(i, totalIter, plikInfo);

    fileName = CASEDATA{i}.ECGfilename.Text;

    if exist(fullfile(ECGDIR, fileName), 'file')
        ECG = loadmat(fullfile(ECGDIR, fileName));
    elseif exist(fullfile(ECGDIR, [fileName '.bsm']), 'file')
        ECG = loadmat(fullfile(ECGDIR, [fileName '.bsm']));
    elseif exist(fullfile(ECGDIR, [fileName '.ecg.medianecg']), 'file')
        ECG = loadmat(fullfile(ECGDIR, [fileName '.ecg.medianecg']));
    elseif exist(fullfile(ECGDIR, [fileName '.medianecg']), 'file')
        ECG = loadmat(fullfile(ECGDIR, [fileName '.medianecg']));
        ...
        if isscalar(BEATS)
            ...
            for j= 1:length(BEATS)
                if isfield(BEATS{j},'medianecg') && ~isempty(BEATS{j}.medianbaselinePoints.Text)
                    ...
                    vBeatFileName = [CASEDATA{i}.ECGfilename.Text '_' num2str(j-1)];
                
                    if exist(fullfile(ECGDIR, [vBeatFileName '.vmedianecg']), 'file')
                        DATAV{iEcgv}.ECG = loadmat(fullfile(ECGDIR, [vBeatFileName '.vmedianecg']));
                    elseif exist(fullfile(ECGDIR, [vBeatFileName '.bsm']), 'file')
                        DATAV{iEcgv}.ECG = loadmat(fullfile(ECGDIR, [vBeatFileName '.bsm']));
                    elseif exist('ECG', 'var')
                        DATAV{iEcgv}.ECG = ECG;
                    end
        ...
        if isscalar(BEATS)
            ...
            for j= 1:length(BEATS)
                if isfield(BEATS{j},'medianecg') && ~isempty(BEATS{j}.medianbaselinePoints.Text)
                    ...
                    aBeatFileName = [CASEDATA{i}.ECGfilename.Text '_' num2str(j-1)];
                
                    if exist(fullfile(ECGDIR, [aBeatFileName '.amedianecg']), 'file')
                        DATAA{iEcga}.ECG = loadmat(fullfile(ECGDIR, [aBeatFileName '.amedianecg']));
                    elseif exist(fullfile(ECGDIR, [aBeatFileName '.bsm']), 'file')
                        DATAA{iEcga}.ECG = loadmat(fullfile(ECGDIR, [aBeatFileName '.bsm']));
                    elseif exist('ECG', 'var')
                        DATAA{iEcga}.ECG = ECG;
                    end
...
%     disp(['reading median ECG file: ' CASEDATA{i}.ECGfilename.Text])   
    ...
    elseif exist(fullfile(ECGDIR, [fileName '.bsm']), 'file')
        ECG = loadmat(fullfile(ECGDIR, [fileName '.bsm']));
    elseif exist(fullfile(ECGDIR, [fileName '.ecg']), 'file')
        ECG = loadmat(fullfile(ECGDIR, [fileName '.ecg']));
    end
