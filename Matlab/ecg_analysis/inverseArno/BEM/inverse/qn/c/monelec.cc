...
#define NEL	    63 
#define distance     0.06
#define V2row        1  /* 0 of 1 */
#define distV2SN     0.12+V2row*distance

int 	ndhk, npnt;
int 	(*dhk)[3]=NULL;
double 	(*pnt)[3]=NULL, dir[3],p[3],p1[3],p2[3],p3[3],n[3],pt[6][3];
double   pl1[3],pr1[3],pf1[3],pl2[3],pr2[3],pf2[3],refpnt[3];
int	nel;
...
void main(int argc, char *argv[])
 {
  ...
  DCONT *h1,*h2;
  double l, m;
  char 	fname[80]="\0",filename[80]="\0",s[80]="\0",line[80]="\0";
  FILE	*file,*f;
  double pos[18],pos2[18];
  double ymin,ymax;
  int cmin,cmax;
  DCONT *c,*vc;
  int nlm,vnlm;
...
*/
  ...
  while (a<1){
    ...
    strcat(s,filename);
    strcat(s," @mon.inp");
    printf("Triplot gebruiken om supra sternal notch te bepalen? (j/n)");
    gets(line);
    if (line[0]=='j')
      system(s);
    ...
    if (!feof(f)){
      fgets(line,80,f);
      sscanf(line,"%lf%lf%lf",&pt[a][0],&pt[a][1],&pt[a][2]);
      a++;
    }
    fclose(f);
    if (a<1){
      printf("Er zitten %d punten in de file, terwijl er 1 nodig is!\n",a);
      printf("Doe het nog eens! (druk op return)\n");
      gets(line);
    }
  }
...
/*  get reference points at left side */
  ...
  p1[0]=0; p1[1]=1; p1[2]=pt[0][2]-ddd+vertshift;
  contour(p1, norm_y, 0, norm_x,0, norm_x, npnt, pnt, ndhk, dhk, 
    &nlmh2, h2, &index);
  routlm(pl1,h2[0].tr,h2[0].l,h2[0].m,npnt,pnt,ndhk,dhk);

  p1[0]=0; p1[1]=1; p1[2]=pt[0][2]-ddd-0.15+vertshift;
  contour(p1, norm_y, 0, norm_x,0, norm_x, npnt, pnt, ndhk, dhk, 
    &nlmh2, h2, &index);
...
*/
  ...
  contour(pl1, pr1, 1, norm_minx,0, norm_minx, npnt, pnt, ndhk, dhk, 
    &nlmh1, h1, &index);
  elecs(nlmh1, h1, 0, index, npnt, pnt, ndhk, dhk, 7, pos, 1, tmp1);

  contour(pl2, pr2, 1, norm_minx,0, norm_minx, npnt, pnt, ndhk, dhk, 
    &nlmh2, h2, &index);
  elecs(nlmh2, h2, 0, index, npnt, pnt, ndhk, dhk, 7, pos, 1, tmp2);
  ...
  assign_el("28",el,2+V2row,elec);
  assign_el("27",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("26",el,2-V2row,elec);
  assign_el("25",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("16",el,2+V2row,elec);
  assign_el("15",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("14",el,2-V2row,elec);
  assign_el("13",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("12",el,2+V2row,elec);
  assign_el("11",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("10",el,2-V2row,elec);
  assign_el("9",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("8",el,2+V2row,elec);
  assign_el("7",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("6",el,2-V2row,elec);
  assign_el("5",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("4",el,2+V2row,elec);
  assign_el("3",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("2",el,2-V2row,elec);
  assign_el("1",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("56",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("40",el,2-V2row,elec);
  assign_el("24",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("23",el,2+V2row,elec);
  assign_el("22",el,1+V2row,elec);
  assign_el("21",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("20",el,1-V2row,elec);
  assign_el("19",el,2-V2row,elec);
  assign_el("18",el,3-V2row,elec);
  assign_el("17",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("39",el,2+V2row,elec);
  assign_el("38",el,1+V2row,elec);
  assign_el("37",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("36",el,1-V2row,elec);
  assign_el("35",el,2-V2row,elec);
  assign_el("34",el,3-V2row,elec);
  assign_el("33",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("55",el,2+V2row,elec);
  assign_el("54",el,1+V2row,elec);
  assign_el("53",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("52",el,1-V2row,elec);
  assign_el("51",el,2-V2row,elec);
  assign_el("50",el,3-V2row,elec);
  assign_el("49",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("63",el,2+V2row,elec);
  assign_el("62",el,1+V2row,elec);
  assign_el("61",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("60",el,1-V2row,elec);
  assign_el("59",el,2-V2row,elec);
  assign_el("58",el,3-V2row,elec);
  assign_el("57",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("47",el,2+V2row,elec);
  assign_el("46",el,1+V2row,elec);
  assign_el("45",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("44",el,1-V2row,elec);
  assign_el("43",el,2-V2row,elec);
  assign_el("42",el,3-V2row,elec);
  assign_el("41",el,4-V2row,elec);
  ...
  contour(p1, n,0,p2, 1, norm_z, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("48",el,0+V2row,elec);

  contour(p1, n,0, p2, 1, norm_minz, npnt, pnt, ndhk, dhk, &vnlm, vc, &index);
  elecs(vnlm, vc, 0, 7, npnt, pnt, ndhk, dhk, 7, pos2, 0, el);
  assign_el("32",el,1-V2row,elec);
  assign_el("31",el,2-V2row,elec);
  assign_el("30",el,3-V2row,elec);
  assign_el("29",el,4-V2row,elec);
