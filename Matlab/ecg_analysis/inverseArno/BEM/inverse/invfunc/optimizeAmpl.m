...
% repopt    = a structure with [.tims]; reps if opt.tims are deps and vice
% versa
%
% OUTPUT:
% score     = [0] if new solution is not better than previous one, [1] if new solution is an improvement.
% amplopt   = same as input structure amplopt, but with new .pot [when score = 1]
...
TST         = gettres_pot(INV,amplopt,depopt,repopt,notchopt);                                                      % Determine how good the initial input fits the data
...
lamb        = amplopt.lambopt;                                                                                      % Set lamb to initial lambdaopt value.
dif_pot     = 0.001;                                                                                                % Change in .pot to produce Splus and Smin.
ampmod      = 4;                                                                                                    % mode for getSfunc.m

Splus       = getSfunc(INV.T, depopt.tims, repopt.tims, INV.SPECS, notchopt.pot, amplopt.pot+dif_pot, ampmod, INV); % Produce TMP's for .pot + dif_pot
Smin        = getSfunc(INV.T, depopt.tims, repopt.tims, INV.SPECS, notchopt.pot, amplopt.pot-dif_pot, ampmod, INV); % Produce TMP's for .pot - dif_pot
Sprime      = (Splus(:,amplopt.usetimes)-Smin(:,amplopt.usetimes))/(dif_tims*2);
SST         = Sprime*Sprime';
...
while TST.tres >= starttres,
    ...
    testopt             = amplopt;
    test.pot            = newpot;                                                                                   % ??
    TST                 = gettres_pot(INV,testopt,depopt,repopt,notchopt);                                          % Determine how good the new input [testopt] fits the data
    ...
    if TST.tres < starttres,                                                                                        % Check if new residual is smaller than in beginning
        if (starttres-TST.tres)/starttres >= INV.stopcrit,                                                          % Check if the improvement is big enough by comparing it to a stop criterion
            ampl.pot    = newpot;                                                                                   % Change ampl.pot to newpot, because of improvement
...
TST             = gettres_pot(INV,amplopt,depopt,repopt,notchopt);                                                  % Calculate final result TST structure for output