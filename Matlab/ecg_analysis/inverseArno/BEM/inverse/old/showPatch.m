...
if isempty(lines),   lines  = loadmat('tims.mcm');      end
if ~isempty(color),  dobar  = 0; isolines = 0;          end
if abs(mindep) > 10, mindep = min(10*floor(mindep/10)); end
...
if posi(3) > .5 && dobar,
    axes('Position',[posi(1)+posi(3)*0.7 posi(2)+posi(4)*0.1 0.25*posi(3) 0.8*posi(4)]); 
    axis off
    caxis([mindep maxdep])
    colorbar;
end
