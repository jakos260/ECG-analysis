...
if ~exist('ElecPos'), error('No ElecPos given!'); end

if ischar(ElecPos),
    switch lower(ElecPos),
        case {'amsbdf'}
            % vl = 66; vr = 67; vf = 65;    % Use (bdf) channel numbers vl66,vr67, vf65 should end up as 63, 64, 65 amsterdam
            vl = 63; vr = 64; vf = 65;      % BDF Biosemi AMS: 257 258 259  
            ...
            if ~all(ismember(standard,SelectChannels)), standard = []; end
            
            leadsys = 'ams';
            
            % chanswitch = [1:62,66,67,65]; % BSM, VL, VR, VF
            % VL63 = 67, vr 64 = 65, vf 65 = 66, maar is hier nog mee geschoven?
...
if ~exist('mla'),
    ...
    for i = 1:size(ElecPos,1),
        for j = 1:size(ElecPos,2),
            if ElecPos(i,j) > 0, mla(end+1,:) = [j,i,ElecPos(i,j)]; end
        end
    end
    leadsys = '';
end