...
function extr=sigplotPO(PHI,label,LAY,funscal,col,leadnos,markt,listextr,linew,stlds)
global sinter PHI2; %hack oostep1
...
for i=2:nplts+1
   ...
   xshift(j)=LAY(i,1)-1;
   if ismember(j,stlds),
        plot(t+xshift(j),funscal*PHI(j,:)+yshift(j),'r','linewidth',linew)
   else,
        plot(t+xshift(j),funscal*PHI(j,:)+yshift(j),col,'linewidth',linew);
        hold on
        plot(t+xshift(j),funscal*PHI2(j,:)+yshift(j),'red','linewidth',linew) %hack oostep1
   end
