...
% compute the matrix of the distributed solid angles SA at observation point obs (size:ntri,3)
% subtended by a set of triangles defined by (VER,ITRI)
% A. van Oosterom; 2003-02-23 
...
% assumes the outward normal of the triangles to be defined by a clockwize rotation of the vertices ITRI(:,[1 2 3]) 
% as as viewed from outside the closed surface; for observation points (obs) inside a closed surface, 
% the SA valuse (in STAR_RADIANS)
% are negative; for external obs, SA is zero; if obs coincides with any vertes of 
% 2012_02_09:  real introduced around log(   )
...
if isempty(k)==0,
   % additional computation required for triangles: k
   S1=R2(k,:)-R1(k,:);
   S2=R3(k,:)-R2(k,:);
   S3=R1(k,:)-R3(k,:);
