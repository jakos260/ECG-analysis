...
function meas=inverse_po(varargin)%GEOM,inittimes,initcase,runcase,usetimes,leads,mode,mudep,murep)
...
% Sprime=(Splus-Smin)/2;
Sprime=(Splus-Smin); % PO: chain rule ^2


%oostep1: scale for rd and wrd
% if ~INV.useWeighedRd 
%     Sprime=Sprime/INV.normphi; % *???
% else
%     Sprime=bsxfun(@rdivide,Sprime,(0.0010 + rms(INV.PHIREF)));
% end
...
if ~INV.useWeighedRd % PO
    M=M/(INV.normphi^2);
end

GTGM = M'*M + muREG;
% GTGM = M + muREG; old
gtres=sum(INV.AMA'.*(Sprime * TST.RES'),2); % origineel PvD

if ~INV.useWeighedRd % PO
    gtres=sum(INV.AMA'.*(Sprime * TST.RES')/INV.normphi,2); % probeersel PO
else % nog niets voor wrd
    gtres=sum(INV.AMA'.*(Sprime * TST.RES'),2); % origineel PvD
end

righths= M' * gtres - muREG * startopt.tims; % PvD origineel
...
while 1 %TST.tres >= starttres, % inner loop
    ...
    if lamb > INV.lambdamax,
        display('lambdamax break');
        break;
    end
%     GTGM_L = GTGM + lamb^2*eye(size(INV.AMA,2)); % Org AvO
            GTGM_L=GTGM+lamb^2*eye(size(INV.AMA,2)).*GTGM; % Exp. PO

%         lastwarn('','');
%         invGTGM_L = inv(GTGM_L);
%         if ~isempty(lastwarn)
%             disp('pinv used');
%             invGTGM_L =pinv(GTGM_L);
%         end
%         deltau = invGTGM_L * righths;
...
if INV.useWeighedRd
    if strcmp(opt.type,'dep')
        ...
        TST.tresold=sqrt(TST.wrd^2+(TST.reg*opt.mu/1000)^2);
        TST.RES=TST.RES.^2./(0.0010 + rms(INV.PHIREF)) % OOSTEP1: real ML, CHECK
    else
        TST.tres=sqrt(TST.wrd^2+(TST.reg*opt.mu)^2);
        TST.tresold=sqrt(TST.wrd^2+(TST.reg*opt.mu/1000)^2);
    ...
    if strcmp(opt.type,'dep')
        TST.tres=sqrt(TST.rd^2+(TST.reg*opt.mu)^2);
        TST.RES=TST.RES.^2/(INV.normphi^2);% OOSTEP1: real ML
    else
        TST.tres=sqrt(TST.rd^2+(TST.reg*opt.mu)^2);
    end
end
