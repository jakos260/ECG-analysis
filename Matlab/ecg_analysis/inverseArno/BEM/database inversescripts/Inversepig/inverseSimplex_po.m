...
function meas=inverseLM_po(varargin)%GEOM,inittimes,initcase,runcase,usetimes,leads,mode,mudep,murep)
...
% gettres_v_vec(opt,keepopt,notchopt,amplopt,INV)

if 1
options=optimset('Algorithm',{'levenberg-marquardt',OPT.DEP.lambopt},'Display', 'iter','FinDiffType','central',...
    'TypicalX',100*ones(size(OPT.DEP.tims)),'FinDiffRelStep',0.01);
x=OPT.DEP.tims;

% fh=@(x)gettres_v_vec(x,OPT.DEP.mu,OPT.REP,OPT.NOT,OPT.AMP,INV);
% [x,resnorm,residual,exitflag,output,lambda] = lsqnonlin(fh,OPT.DEP.tims,[],[],options);

options=optimset('Display','iter','MaxIter',10000);
fh=@(x)gettres_v_tres(x,OPT.DEP.mu,OPT.REP,OPT.NOT,OPT.AMP,INV);
[x,fval,exitflag,output] = fminsearch(fh,OPT.DEP.tims,options);%,'Display','iter');

OPT.DEP.tims=x;
end

if 0 % old optimization
...
if strcmp(opt.type,'dep')
    ...
    fplus=gettres_v_vec(opt_tims+1,opt_mu,keepopt,notchopt,amplopt,INV);
    fmin=gettres_v_vec(opt_tims-1,opt_mu,keepopt,notchopt,amplopt,INV);
    startopt=opt;
...
%% =======================================================================
    function tres=gettres_v_tres(opt_tims,opt_mu,keepopt,notchopt,amplopt,INV)
        tres=rms(gettres_v_vec(opt_tims,opt_mu,keepopt,notchopt,amplopt,INV));
        


%% =======================================================================

function resrd=gettres_v_vec(opt_tims,opt_mu,keepopt,notchopt,amplopt,INV)

% if strcmp(opt.type,'dep')
    TST.S=getS(INV.T,opt_tims,keepopt.tims,INV.SPECS,notchopt.pot,amplopt.pot,INV.mode,INV);
% else %pol=='rep',
%     TST.S=getS(INV.T,keepopt.tims,opt_tims,INV.SPECS,notchopt.pot,amplopt.pot,INV.mode,INV);
% end

TST.PHIA=lowpassma(INV.AMA*TST.S, INV.lpass);
TST.RES=INV.PHIREF - TST.PHIA(1:size(INV.PHIREF,1),1:size(INV.PHIREF,2));
resrd=INV.PHIREF - TST.PHIA(1:size(INV.PHIREF,1),1:size(INV.PHIREF,2));
resrd=resrd(:)+opt_mu*norm(INV.REGOP*opt_tims);
