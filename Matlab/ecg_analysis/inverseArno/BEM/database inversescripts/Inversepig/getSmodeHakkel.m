function S=getSmodeHakkel(T,dep,rep,SPECS,mode,varargin)
...
GEOM=varargin{1};
for i=1:length(dep)
    if dep(i) >= 0 && dep(i) < size(Sscale,2)
        depNeigh = dep( GEOM.ADJsurf(i,:) > 0 );
        b = (depNeigh - dep(i)); %/1.73;
        slope = 1./b;
        depi = round(dep(i));
        bi = round(b);
        ran = bi + depi;

        maxt = max(max(ran),depi);
        mint = min(depi,min(ran));
        t = mint:maxt;
        ti = t+1;
        factor = 1 / length(b);
        Sscale(i,1:min(size(S,2),max(ran)+1)) = 0;

        tii = find(ti>0 & ti <= size(S,2));
        s=bsxfun(@times,abs(slope),(ones(length(depNeigh),1) *  t  - (min(depi,round(depNeigh)) * ones(1,length(t)))));
        s(s>1)=1;
        s(s<0)=0;
        s = sum(s * factor);
        Sscale(i,ti(tii)) = Sscale(i,ti(tii)) + s(tii);
        if size(Sscale,2) > size(S,2)
            a=1;
        end
    %         for j =1:length(slope)          
    %             s = abs(slope(j)) * (ones(length(depNeigh),1) *  t  - (min(depi,round(depNeigh)) * ones(1,length(t)))); 
    %             Sscale(i,tii) = Sscale(i,tii) + s(tii);
    %         figure(1);clf;plot(S(i,:));hold on; plot(Sscale(i,:),'r');plot(ss,'g')
    %         drawnow
    %         end       
    end
end
