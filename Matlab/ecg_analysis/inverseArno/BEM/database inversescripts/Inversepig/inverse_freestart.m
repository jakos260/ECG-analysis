...
function meas=inverse_freestart(varargin)%GEOM,inittimes,initcase,runcase,usetimes,leads,mode,mudep,murep)
...
INV.weight=[];
if doWeight
    INV.noiseLevel = mean(rms(GEOM.BSM(:,GEOM.specs(3):end)-lowpassma(GEOM.BSM(:,GEOM.specs(3):end),21)));      
    rrms=rms(INV.PHIREF);
    INV.weight = INV.noiseLevel./ rrms;
    INV.weight(rrms==0) = 0;
    INV.weight(500:end)=min(INV.weight(INV.weight>0));
    a = INV.weight(1:GEOM.specs(3)-GEOM.specs(2)+1) ./ (sum(INV.weight(1:GEOM.specs(3)-GEOM.specs(2)+1)));
    b = INV.weight(GEOM.specs(3)-GEOM.specs(2)+2:end) ./ (sum(INV.weight(GEOM.specs(3)-GEOM.specs(2)+2:end)));
    INV.weight = [a b];
...
if ~isempty(INV.weight)    
    TST.RES = bsxfun(@times,TST.RES,INV.weight); % oostep1: 20121015 much faster
    rd=norm(TST.RES,'fro')/INV.normphiWeighed; %NOTE: unfiltered rd
else
    rd = TST.rd;
end
...
if strcmp(opt.type,'dep')
    TST.tres=sqrt(rd^2+(TST.reg*opt.mu)^2);
else
    TST.tres=sqrt(rd^2+(TST.reg*opt.mu)^2);
end
