    ...
    while pp<=nargin
        if ischar(varargin{pp})
            ...
            switch key
                ...
                case 'initdep'
                    useInitdep       = varargin{pp+1};pp=pp+2;                    
                case 'initialacttime'
                    SCAN.initialActTime       = varargin{pp+1};pp=pp+2;
                otherwise
...
[initialActTime,SCAN.termtime] = calcSlowActTimes( SCAN.PSIREF(:,1:GEOM.SPECS.qrsduration) );
if isempty(SCAN.initialActTime)
    SCAN.initialActTime=initialActTime;
end
...
sinks=[];
if useInitdep > 0
    dep =  calcDepolarization(SCAN, useInitdep);
    bestdep   = dep;
    bestfoci  = useInitdep;
else
    dep       = [];
...
while  nrClust < SCAN.clusters ||  (max(bestdep) - min(bestdep)) > SCAN.qrsduration + 15 
    if length(bestfoci) > 1
%         dep = delayFoci(GEOM,SCAN,dep,bestfoci);   
    end
...
if isempty(initdep) && ~SCAN.isSinus
    maxt = max(60,floor(SCAN.qrsduration / 2));
    %     maxtEndo = floor(SCAN.qrsduration / 2);
%     maxtEpi = floor(SCAN.qrsduration / 3); 
else
    maxt = SCAN.qrsduration;
%     maxtEndo = floor(SCAN.qrsduration / 1);
%     maxtEpi = floor(SCAN.qrsduration / 1); 
...
%     if GEOM.endoVER(inode)
%         maxt = maxtEndo;
%     else
%         maxt = maxtEpi;
%     end
    if isempty(initdep)
        dep = calcDepolarization(SCAN, GEOM,maxt,inode);
    else
...
function depBetter = delayFoci(GEOM,SCAN,depIn,foci)

deps(1,:) = calcDepolarization(SCAN, foci(1));
for i=2:length(foci)
    inode = foci(i);
    dep = min([deps(i-1,:)' + SCAN.DIST2W(:,inode) / SCAN.maxVelocity, deps(i-1,:)'],[],2);
    if max(dep) < SCAN.qrsduration
        dep = dep * (SCAN.qrsduration / max(dep));
    end
...
maxt= SCAN.qrsduration;
PSIA =lowpassma(SCAN.AMA*getSmode(ones(length(GEOM.VER),1) * (1:maxt),depIn,depIn+400, GEOM.SPECS, SCAN.scanmode,GEOM),SCAN.lpass);
rds0 = norm(SCAN.PSIREF(:,1:maxt) - PSIA,'fro')/norm(SCAN.PSIREF(:,1:maxt),'fro');
cors0 =  compCor(SCAN.PSIREF(:,1:size(PSIA,2)),PSIA,1);

ifoci = find(depIn(foci) == min(depIn(foci)));
ifoci=ifoci(1);

depOther = deps;
depOther(ifoci,:)=[];
depOther = min(depOther,[],1);
depBetter=[];
for d=0:5:20
    dep = min(depOther, deps(ifoci,:)+d);
    maxt= SCAN.qrsduration;
    PSIA =lowpassma(SCAN.AMA*getSmode(ones(length(GEOM.VER),1) * (1:maxt),dep,dep+400, GEOM.SPECS, SCAN.scanmode,GEOM),SCAN.lpass);
    rds = norm(SCAN.PSIREF(:,1:maxt) - PSIA,'fro')/norm(SCAN.PSIREF(:,1:maxt),'fro');
    cors =  compCor(SCAN.PSIREF(:,1:size(PSIA,2)),PSIA,1);
    if cors > cors0
        depBetter = dep;
    end
...
% % difference
% if SCAN.anisotropyRatio < 1
%     dep = SCAN.DIST(:,inode) / SCAN.initialVelocity;
%     depw = SCAN.DIST2W(:,inode) / SCAN.initialVelocity;
%     dt0= min(20,SCAN.initialActTime);
%     depAbove = find(dep >  dt0 );
%     dep(depAbove) = dt0 + (depw(depAbove) - dt0) * SCAN.initialVelocity / min(SCAN.maxVelocity,(max(depw) - dt0) * SCAN.initialVelocity/ (SCAN.qrsduration - dt0) );
% else
...
%% ========================================================================
function bestdep = calcDepolarization(SCAN,GEOM,maxt,inode)

if 0 % initial ansiotropy 2.5
    dist = SCAN.initialActTime * SCAN.initialVelocity;
    ADJ=GEOM.ADJ25/SCAN.initialVelocity;
    others =find(GEOM.DIST(inode,:)>dist);
    ADJ(others,others) = GEOM.ADJ2W(others,others)/SCAN.maxVelocity;
    dep = graphdistone(ADJ,inode);
    if max(dep) < GEOM.SPECS.qrsduration
        dep = dep * GEOM.SPECS.qrsduration / max(dep);
    end
    bestdep = dep;
    rep= dep + 400;
    ...
    if cor > 0
        for dt=2:2:dist
            dist = SCAN.initialActTime * SCAN.initialVelocity - dt;
            ADJ=GEOM.ADJ25/SCAN.initialVelocity;
            others =find(GEOM.DIST(inode,:)>dist);
            ADJ(others,others) = GEOM.ADJ2W(others,others)/SCAN.maxVelocity;
            dep = graphdistone(ADJ,inode);
    ...
    return
elseif 1 % initial ansiotropy 1
    % the propagtion velocity is assumed to be homogenoeus initially. Probably
    % the numerical accuracy of the method used is not high enough to tell the
    % difference
    if SCAN.anisotropyRatio < 1
        dep = GEOM.DIST(:,inode) / SCAN.initialVelocity;
        depw = SCAN.DIST2W(:,inode) / SCAN.initialVelocity;
        dt0= min(60,SCAN.initialActTime);
        depAbove = find(dep >  dt0 );
        dep(depAbove) = dt0 + (depw(depAbove) - dt0) * SCAN.initialVelocity / min(SCAN.maxVelocity,(max(depw) - dt0) * SCAN.initialVelocity/ (SCAN.qrsduration - dt0) );
    else
        dep = SCAN.DIST2W(:,inode) / SCAN.initialVelocity;
    ...
    if cor > 0
        for dt=2:2:50%min(40,SCAN.initialActTime)
            initialActTime = SCAN.initialActTime -dt;      
            % the propagtion velocity is assumed to be homogenoeus initially. Probably
            % the numerical accuracy of the method used is not high enough to tell the
            % difference
            if SCAN.anisotropyRatio < 1
                dep = GEOM.DIST(:,inode) / SCAN.initialVelocity + dt;
...
elseif 0 % equal all
    ...
    % difference
    if SCAN.anisotropyRatio < 1
        dep = GEOM.DIST2W(:,inode) / SCAN.initialVelocity;
        depw = SCAN.DIST2W(:,inode) / SCAN.initialVelocity;
        dt0= min(60,SCAN.initialActTime);
        depAbove = find(dep >  dt0 );
        dep(depAbove) = dt0 + (depw(depAbove) - dt0) * SCAN.initialVelocity / min(SCAN.maxVelocity,(max(depw) - dt0) * SCAN.initialVelocity/ (SCAN.qrsduration - dt0) );
    ...
    if cor > 0
        for dt=2:2:50%min(40,SCAN.initialActTime)
            ...
            % difference
            if SCAN.anisotropyRatio < 1
                dep = GEOM.DIST2W(:,inode) / SCAN.initialVelocity + dt;
                depw = SCAN.DIST2W(:,inode) / SCAN.initialVelocity + dt;
                dt0= min(60,SCAN.initialActTime);
                depAbove = find(dep >  dt0 );
                dep(depAbove) = dt0 + (depw(depAbove) - dt0) * SCAN.initialVelocity / min(SCAN.maxVelocity,(max(depw) - dt0) * SCAN.initialVelocity/ (SCAN.qrsduration - dt0) );
