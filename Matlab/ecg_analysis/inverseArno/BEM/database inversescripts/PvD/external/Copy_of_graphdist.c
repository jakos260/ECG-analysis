...
void showHelp(void)
{
	mexPrintf("GRAPHDIST Version: 2.0\nCalculates the shortest paths in a fully connected graph\n\n");
	mexPrintf("SYNTAX\n");
...
{
	int i,j,i2,i3,k;
	double vm,v1[3],v2[3],len=-1,temp;
	...
	for (i=0;i<ndhk;i++)
	{
		...
		{
			len =ADJ[i1+npnt*i2]+ADJ[j2+npnt*i2];
			for (k=1;k<=100;k++)
			{
				for (j=0;j<3;j++)
				{
					vm=pnt[i2+j*npnt]+(pnt[i3+j*npnt]-pnt[i2+j*npnt])*(k/100.0);
					v1[j]=vm-pnt[i1+j*npnt];
					v2[j]=vm-pnt[j2+j*npnt];				
				}
				temp=norm(v1)+norm(v2);
				if (temp<len)
					len=temp;
			}
			return 	len;
...
void calcRibNorm(int i1,int i2,double *n)
{
	int i,j,t[100],tt;
	double v1[3],v2[3],vn[3],len,vntot[3];

	tt=0;
	for (i=0;i<ndhk;i++)
	{
		if ((i1==dhk[i][0] && i2==dhk[i][1]) || (i1==dhk[i][0] && i2==dhk[i][2]) ||
			(i1==dhk[i][1] && i2==dhk[i][0]) || (i1==dhk[i][1] && i2==dhk[i][2]) ||
			(i1==dhk[i][2] && i2==dhk[i][1]) || (i1==dhk[i][2] && i2==dhk[i][0]))
		{
			t[tt]=i;
			tt++;
		}
	}
	vntot[0]=0;vntot[1]=0;vntot[2]=0;
	for (i=0;i<tt;i++)
	{
		for (j=0;j<3;j++)
		{
			v1[j]=pnt[dhk[t[i]][0]+j*npnt]-pnt[dhk[t[i]][1]+j*npnt];
			v2[j]=pnt[dhk[t[i]][0]+j*npnt]-pnt[dhk[t[i]][2]+j*npnt];
		}
		veccross(v1,v2,vn);
		vntot[0]+=vn[0];vntot[1]+=vn[1];vntot[2]+=vn[2];
	}
	len=norm(vntot);
	for (j=0;j<3;j++)
		n[j]=vntot[j]/len;
...
void calcLocalNorm(int i1,double *n)
{
	int i,j,t[100],tt;
	double v1[3],v2[3],vn[3],len,vntot[3];
	...
	for (i=0;i<ndhk;i++)
	{
		if (i1==dhk[i][0] || i1==dhk[i][1] || i1==dhk[i][2]) 
		{
			t[tt]=i;
			tt++;
		}
...
void wallDist(void)
{
	...
	double r[3];
 	for (j1=0;j1<npnt;j1++)
		...
		{
			if (!ADJ[j1+npnt*j2] && nodeview(j1,j2)==1)
			{
				for (i=0;i<3;i++)
...
/*						calcLocalNorm(k,rn2);
						...
						{
							for (j1=0;j1<3;j1++)							
								v[j1]= pnt[i+j1*npnt] - pnt[k+j1*npnt];
							ADJ[i+npnt*k]=norm(v);
							ADJ[k+npnt*i]=ADJ[i+npnt*k];							
						}
 */
					}
			}
...
// 			if (DD[i+j*npnt]!=0)
// 			{
// 				ADJ[i+j*npnt]=DD[i+j*npnt];
// 				ADJ[j+i*npnt]=DD[j+i*npnt];
// 			}
	mxFree(DD);
}
