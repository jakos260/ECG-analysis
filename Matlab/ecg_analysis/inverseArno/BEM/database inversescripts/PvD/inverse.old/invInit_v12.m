...
function GEOM=invInit_v12(varargin)
...
while pp<=nargin
	if ischar(varargin{pp})
		...
		switch key
            ...
            case 'useleads'
                useLeads = varargin{pp+1};pp=pp+2;
...
% create always a 12 leads
GEOM.LAY = [ 3 3 0;...
           1 1 GEOM.v12(1); 1 2 GEOM.v12(2); 1 3 GEOM.v12(3);...
           2 1 GEOM.v12(4); 2 2 GEOM.v12(5); 2 3 GEOM.v12(6);...
           3 1 GEOM.v12(7); 3 2 GEOM.v12(8);3 3 GEOM.v12(9)];
...
GEOM.BSM = GEOM.BSM(4:end,:);
GEOM.BSM(1:3,:) = GEOM.BSM(1:3,:)  /1.5;
if isempty(GEOM.BSM)
...
if ~isempty(strfind(layfile,'nim')) && size(GEOM.BSM,1) == 64
    GEOM.BSM = [GEOM.BSM; zeros(size(GEOM.BSM(1,:)))];    
end

if size(GEOM.BSM,1)~= length(GEOM.TVER) 
    T=intripol(GEOM.TVER,GEOM.TITRI,GEOM.v12);
    GEOM.BSM = T * GEOM.BSM;
end
...
% savemat(['results\' GEOM.subject '_wct_v2tama.ama'],GEOM.AMA);
savemat(['results\' GEOM.subject '_v12wct.bsm'],GEOM.BSM);
if 0
    elNodes = GEOM.v12; %loadmat([heartpath 'elnodes.mat']');
    AMA = GEOM.AMA;
    BSM= GEOM.BSM;
    VER=GEOM.TVER;
    ITRI=GEOM.TITRI;
...
if exist([distpath type 's.dst3d'],'file')
	...
	error('no distance matrices defined');
end
if GEOM.anisotropyRatio ~= 2
    GEOM.ADJ2W=calcAnisADJ(GEOM,GEOM.anisotropyRatio);
end
%%
