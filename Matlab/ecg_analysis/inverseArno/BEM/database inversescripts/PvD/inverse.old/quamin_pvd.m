...
[yest,G]=rfunc_pvd(x,pinit,1,funtype);
res=y-yest;
...
while loop1>=1,
   ...
   while break1==0,
       lambda=2*lambda;
       if lambda > 1.e+10, break1=1; lambda;   bestnorm=testnorm; break, end
       ...
       % test new estimate
       [yest]=rfunc_pvd(x,testp,0,funtype);
       res=y-yest;
       testnorm=norm(res);
...
%          disp(sprintf('niter   relres\n %3d   %0.4g',[iter relres]))
    ...
    lambda=lambda/4;
    [yest,G]=rfunc_pvd(x,parms,1,funtype);
end
return;
