function varargout=MIActivation(varargin) 
...
if length(MIarea)>1 && length(MIarea)~=length(node)
	warning('Number of radii for the ischemic areas differs from the number ischemic areas only the first one is used');
	MIarea=MIarea(1);
end
...
if transmural==0
	disp(['Area ' num2str(MIarea) ' velocity reduction ' num2str(cvFactor) '  APD shortning ' num2str(APDshort) ' single card only' '   velocity reset to: ' defaults blocktext]);
else
	disp(['Area ' num2str(MIarea) ' velocity reduction ' num2str(cvFactor) '  APD shortning ' num2str(APDshort) ' transmural (factor ' num2str(transmural) ')' '   velocity reset to: ' defaults blocktext]);
end
...
for i=1:length(node)
	if transmural==0	
		imi=find(VENTR.DIST2D(:,node(i))< MIarea(min(length(MIarea),i)));
	elseif transmural==1
		imi=find(VENTR.DIST(:,node(i))< MIarea(min(length(MIarea),i)));
	elseif transmural > 0
		tn=~VENTR.endoVER(node(i));
		oposite=find(VENTR.ADJ(node(i),:) == min(VENTR.ADJ(node(i),VENTR.ADJ(node(i),:)>0 & VENTR.endoVER==tn)));
		imi=find(VENTR.DIST(:,node(i))< MIarea(min(length(MIarea),i))*transmural(min(length(transmural),i)));
		imi(VENTR.endoVER(imi)~=VENTR.endoVER(node(i)))=[];
		imio=find(VENTR.DIST2D(:,oposite)< MIarea(min(length(MIarea),i))*transmural(min(length(transmural),i)));
		imi=unique([imi; imio]);
	else %if transmural < 0
		tn=~VENTR.endoVER(node(i));
		oposite=find(VENTR.ADJ(node(i),:) == min(VENTR.ADJ(node(i),VENTR.ADJ(node(i),:)>0 & VENTR.endoVER==tn)));
		imi=find(VENTR.DIST(:,node(i))< MIarea(min(length(MIarea),i))*abs(transmural(min(length(transmural),i))));
		imi(VENTR.endoVER(imi)==VENTR.endoVER(node(i)))=[];
		imio=find(VENTR.DIST2D(:,oposite)< MIarea(min(length(MIarea),i))*abs(transmural(min(length(transmural),i))));
		imi=unique([imi; imio]);
		node = oposite;
	end
	eval(['imi_' num2str(i) '=imi;']);
	timi=[timi; imi];
end
imi=unique(timi);
...
if doHighVelocities % default
	...
	for i=1:length(VENTR.ADJ),
		for j=1:length(VENTR.ADJ),
			if purkinjever(i) && purkinjever(j), Vgeom(i,j)=purkinjevelocity;	end; 
		end; 
	end
	...
	for i=1:length(Vgeom)
		if VENTR.endoVER(i)
	        Vgeom(i,VENTR.endoVER==1)= Vgeom(i,VENTR.endoVER==1)*2; % purkinje velocities over the wall 2 m/s
		end
	end
...
%% Determine nodes at which the sinus wavefront (repV) has a local minimum (locmin)
rlocmin=[];
for i=1:length(VENTR.VER)
	if all(repV(VENTR.ADJ(:,i)~=0 & VENTR.ADJ(:,i)<locMinArea)-repV(i)>0)
		rlocmin=[rlocmin i];
	end
end

%% Keeplocmin
% Determine nodes at which the MI wavefront (depMI) has a local minimum
Keeplocmin=[];
for i=1:length(locmin)
	if isempty(find(imi==locmin(i),1)) 
		Keeplocmin=[Keeplocmin locmin(i)];
	end
end

%% 
% Determine nodes for which the time has to be adapetd because they are in
% the ischemic area (remloc)
remloc=[];
for i=1:length(locmin)
	if isempty(find(locmin(i)==Keeplocmin,1))
		remloc=[remloc locmin(i)];
	end
end
...
if ~isempty(imi)
	if cvFactor<=-1 % colmplete block
		...
		APD=repV-depV;
		fact=calcFact(VENTR.DIST(node,imi),MIarea);
		if length(APDshort)==1
				APD(imi)=APD(imi)-(APDshort(min(length(APDshort),i))*(fact'));
		else
			for i=1:length(APDshort)
				eval(['timi=imi_' num2str(i)]);
				fact=calcFact(VENTR.DIST(node,imi),MIarea(min(length(MIarea),i)));
				APD(timi)=APD(timi)-(APDshort(i)*(fact')); 
			end
		end
		repp=depp+APD;
	end
...
function fact=calcFact(dists,radius)
