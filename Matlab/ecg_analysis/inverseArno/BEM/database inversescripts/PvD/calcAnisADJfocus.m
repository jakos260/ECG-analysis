...
buur=graphdist(GEOM.ITRI,GEOM.VER,4);

% if anisotropyRatio==1
% 	ADJ(inode,buur(inode,:)==0)=ADJ(inode,buur(inode,:)==0)*.3333;
% 	ADJ(buur(:,inode)==0,inode)=ADJ(buur(:,inode)==0,inode)*.3333;	
% 	ADJ2=ADJ;
% 	return;
% end
	...
	for j=i:length(ADJ2)
		if ADJ2(i,j)>0 && type(i)~=type(j)
			...
			if i==focus || focus==j
			    ...
			    d=sqrt(min([d1,d2])^2+((0.6)*anisotropyRatio*mean([mdi,mdj]))^2);				
			else
	            d1=anis*norm3d(VER(j,:)-pnti);
		        d2=anis*norm3d(VER(i,:)-pntj);
			    d=sqrt(min([d1,d2])^2+(anisotropyRatio*mean([mdi,mdj]))^2);
			end
%                 ADJ2(i,j)=min([d1,d2])+anisotropyRatio*mdj;
%                 ADJ2(j,i)=min([d1,d2])+anisotropyRatio*mdi;
            ADJ2(i,j)=d; ADJ2(j,i)=d;                
		end
	end
end
...
% tic;dist2=graphdist(ADJ2);toc
% GEOM.DIST2W=dist2;
	

function anis=calcVlongwall(anisotropyRatio)

alpha=0:pi/30:pi*2/3;  % assuem roation of 120 degrees
