...
function GEOM=prepare_geom(GEOM,fileout,saveFile)
...
rrms=rms(PSI);
rrms=2*rrms/min(1,max(rrms))*LAY(1,2); % opostep1 limit scale
nt=length(rrms);
...
GEOM.specs=specs;    
GEOM.pS=specs(6:end);
GEOM.qrsduration=GEOM.specs(3)-GEOM.specs(2)+1; % endqrs - onsetqrs  
