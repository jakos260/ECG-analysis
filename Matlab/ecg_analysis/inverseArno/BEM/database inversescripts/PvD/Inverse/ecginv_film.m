...
useSurfLapl=1;
maxiter=100;  
if strfind(GEOM.subject,'ppd1'),maxiter=15;end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PREPARE INPUT DATA
...
if domonitor==1
	figure(3);clf;sigplot_p(INV.PHI,GEOM.subject,GEOM.LAY,sigscal,'b',1,0,1);
...
FF=[];FF1=[];
GG=[];
while iter<maxiter && (depscore || repscore) && TST.rd>0.11
	dt=ceil(toc/20);
	dt=2;
% 	figure(13);showAtria2(GEOM.VER,GEOM.ITRI,OPT.DEP.tims ,OPT.REP.tims ,'view',[-55 50],'do2',1);drawnow		
	hdep=figure(13);showAtria2(GEOM.VER,GEOM.ITRI,OPT.DEP.tims ,OPT.REP.tims ,'do2',1,'max',[0 105 270 330],'endo',GEOM.endoVER);drawnow		
	annotation('textbox',[0. 0.9 0.3 0.05],'string',['# ' num2str(iter) ],'edgecolor','none','FitHeightToText','off','Fontsize',12');
	saveas(hdep,['figs\present\optimize_front' num2str(iter) '.png']);	
% 	annotation('textbox',[0.7 0.92 0.3 0.05],'string',['run: ' num2str(iter) '   rd:' num2str(TST.rd,2) ])	
	if ~isempty(FF)
		for pp=1:dt
			FF=[FF getFrame(hdep)];
		end
	...
	hdep=figure(13);showAtria2(GEOM.VER,GEOM.ITRI,OPT.DEP.tims ,OPT.REP.tims ,'do2',1,'max',[0 105 270 330],'view',[-49 57],'endo',GEOM.endoVER);drawnow			
	annotation('textbox',[0. 0.9 0.3 0.05],'string',['# ' num2str(iter) ],'edgecolor','none','FitHeightToText','off','Fontsize',12');	
	saveas(hdep,['figs\present\optimize_base' num2str(iter) '.png']);
	if ~isempty(FF1)
		for pp=1:dt
			FF1=[FF1 getFrame(hdep)];
		end
	else
		FF1=getFrame(hdep);
	end
	...
	hECG=figure(114);leadv16(INV.BSM(:,1:size(TST.PHIA,2)),TST.PHIA , 'leadsys',GEOM.leadname,'max',[-3 3]);drawnow	
% 	annotation('textbox',[0. 0.95 0.01 0.05],'string',['# ' num2str(iter) '       rd:' num2str(TST.rd,2) ],'edgecolor','none','FitHeightToText','on','Fontsize',14');	
	annotation('textbox',[0. 0.92 0.12 0.08],'string',sprintf('%s\n%s',['# ' num2str(iter) ],[ 'rd:' num2str(TST.rd,2) ]),'edgecolor','none','FitHeightToText','off','Fontsize',12,'fontweight','normal','BackgroundColor','w');	
	saveas(hECG,['figs\present\optimize_ECG' num2str(iter) '.png']);
	...
	for loop=loopstart:nloop
		...
		if domonitor==1
			...
			figure(3);   clf
			sigplot_p(INV.PHI(:,1:INV.usetimes),runcase,GEOM.LAY,sigscal,'b',1,0,1);
			sigplot_p(TST.PHIA(:,1:INV.usetimes),runcase,GEOM.LAY,sigscal,'r',0,0,0);
...
if domonitor
	...
	subplot(2,1,1);plot(INV.PHI(:,1:INV.usetimes)');title('measured');
	subplot(2,1,2);plot(TST.PHIA(:,1:INV.usetimes)');title('simulated')
...
global dodeprep
if dodeprep
S=getSvplateau(T,dep,rep,p,scaleAmpl,mode);
else
% S=getSvplateau(T,dep,rep,p,notchpot,mode);
S=getSmode(T,dep,rep,p,notchpot,5);
end