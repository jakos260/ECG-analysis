...
DATA.BSM_low200 = DATA.BSM; %lowpassma(DATA.BSM,1/(200*DATA.sampT));
...
if OPTIONS.zeromean==1
    if isempty(OPTIONS.zmchannels)
        ...
        DATA.BSM_low200 = (DATA.BSM_low200);
    else
...
%     qrss(qrss < 300 )=[];
    qrss = find(rrms > mean(rrms) + 1 * std(rrms) );
    dqrss = [diff(qrss) 1];
    qrss(dqrss<150) =[];
...
if ~isempty(DATA.BEATS)
    ...
    DATA.BSMOUT = DATA.BSM_low200;%zeromean(DATA.BSM_low200); % OUTPUT BY DEFINITION ZEROMEAN
%     BASEL=spline(zerotims,DATA.BSMOUT (:,zerotims),1:size(DATA.BSMOUT ,2));
%     DATA.BSMOUT = DATA.BSMOUT  - BASEL;
    
else
    DATA.BSMOUT = DATA.BSM_low200;%zeromean(DATA.BSMfilt); % OUTPUT BY DEFINITION ZEROMEAN
end
