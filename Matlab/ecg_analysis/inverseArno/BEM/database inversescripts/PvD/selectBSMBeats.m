% Wrapper function for selectBSMBeat. Input should be the BSM data file and
% the used layout (the file should have been read). Additionally the sample
    ...
    if usemean && sum(DATA.SELBEATS) == 0
        ...
        for i=1:length(DATA.SELBEATS)
            if DATA.SELBEATS(i)
                ...
                if ~isempty(filename)
                    savemat([filename '_beat' num2str(i) '.beatecg'], BSMcorrect);
                end
                k = k+1;
            end
        end    
