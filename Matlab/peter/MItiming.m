function varargout=MItiming(varargin) 
...
if cvFactor==0,	cvF=1e5;else cvF=cvFactor;end
...
elseif transmural==0	
	imi=find(VENTR.DIST2D(:,node)< MIarea);
elseif transmural==1
	imi=find(VENTR.DIST(:,node)< MIarea);
else %if transmural > 1
	tn=~VENTR.endoVER(node);
	oposite=find(VENTR.ADJ(node,:) == min(VENTR.ADJ(node,VENTR.ADJ(node,:)>0 & VENTR.endoVER==tn)));
	imi=find(VENTR.DIST2D(:,node)< MIarea);
	imio=find(VENTR.DIST2D(:,oposite)< MIarea*transmural);
	imi=unique([imi; imio]);
end
...
if doHighVelocities % default
	...
	Vgeom=conduction_velocity*VENTR.ADJ./VENTR.ADJ;
    Vgeom(isnan(Vgeom))=0;
	adjactmi=ADJW;
	adjactmi(Vgeom>Vdep)=abs(Tdep(Vgeom>Vdep));
end
...
rKeeplocmin=[];
for i=1:length(rlocmin)
	if isempty(find(imi==rlocmin(i),1)) 
		rKeeplocmin=[rKeeplocmin rlocmin(i)];
	end
end
%% 
...
rremloc=[];
for i=1:length(rlocmin)
	if ~isempty(find(rlocmin(i)==imi,1))
		rremloc=[rremloc rlocmin(i)];
	end
end
