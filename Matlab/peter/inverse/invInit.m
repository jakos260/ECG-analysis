...
group=[];
if ispc
    baseDir = 'C:\Users\peter\Documents\Data\geometries\'; %Peter van Dam's laptop
else
...
disp(['loading model:  ' heartpath type '.tri' ] )
...
if isempty(strfind(type,'ventricles')) && exist([heartpath  '_atria.tri'])
...
elseif exist( [heartpath,'_rho.tri'],'file')
...
if exist( [heartpath,'_thorax.tri'],'file')==2
    ...
    elseif ~isempty(strfind(layfile,'ams')) && exist( [heartpath,'_thoraxAmsterdam_33.tri'],'file') ==2
        [GEOM.TVER,GEOM.TITRI]=loadtri([heartpath,'_thoraxAmsterdam_33.tri']);        
    elseif ~isempty(strfind(layfile,'ams')) && exist( [heartpath,'_thoraxAmsterdam.tri'],'file') ==2
...
elseif ~isempty(strfind(layfile,'arnhem')) && exist( [heartpath,'_thorax99_Arnhem.tri'],'file') ==2
    A=loadmat([heartpath type '_edl.mat']);    
elseif ~isempty(strfind(layfile,'ams65')) && exist( [heartpath,'_thorax_Amsterdam.vedl'],'file') ==2 && ~isempty(strfind(type,'ventricles'))
    A=loadmat([heartpath '_thorax_Amsterdam.vedl']);    
elseif ~isempty(strfind(layfile,'ams65')) && exist( [heartpath,'_thoraxAmsterdam.vedl'],'file') ==2 && ~isempty(strfind(type,'ventricles'))
    A=loadmat([heartpath '_thoraxAmsterdam.vedl']);    
elseif ~isempty(strfind(layfile,'ams65_33')) && exist( [heartpath,'_thorax_Amsterdam_33.vedl'],'file') ==2 && ~isempty(strfind(type,'ventricles'))
    A=loadmat([heartpath '_thorax_Amsterdam_33.vedl']);    
elseif ~isempty(strfind(layfile,'ams65_33')) && exist( [heartpath,'_thoraxAmsterdam_33.vedl'],'file') ==2 && ~isempty(strfind(type,'ventricles'))
    A=loadmat([heartpath '_thoraxAmsterdam_33.vedl']);            
elseif ~isempty(strfind(layfile,'ams65_33')) && exist( [heartpath,'_thorax_Amsterdam_33.aedl'],'file') ==2 && ~isempty(strfind(type,'atria'))
    A=loadmat([heartpath '_thorax_Amsterdam_33.aedl']);    
elseif ~isempty(strfind(layfile,'ams65_33')) && exist( [heartpath,'_thoraxAmsterdam_33.aedl'],'file') ==2 && ~isempty(strfind(type,'atria'))
    A=loadmat([heartpath '_thoraxAmsterdam_33.aedl']);            
elseif ~isempty(strfind(layfile,'ams65')) && exist( [heartpath,'_thorax_Amsterdam.aedl'],'file') ==2 && ~isempty(strfind(type,'atria'))
    A=loadmat([heartpath '_thorax_Amsterdam.aedl']);    
elseif ~isempty(strfind(layfile,'ams65')) && exist( [heartpath,'_thorax_Amsterdam.aedl'],'file') ==2 && ~isempty(strfind(type,'atria'))
    A=loadmat([heartpath '_thorax_Amsterdam.aedl']);    
elseif ~isempty(strfind(layfile,'lds')) && exist( [heartpath,'_thorax12_lead.tri'],'file') == 2
    GEOM.AMAv12 = 40*loadmat([heartpath '_thorax_12 lead.vedl']);
    A=loadmat([heartpath type '_v12edl.mat']);
elseif exist([heartpath type '_edl.mat'],'file')
    A = loadmat([heartpath type '_edl.mat']);
else
    if strfind(type,'ventricles') 
        A= loadmat([heartpath '_thorax.vedl']);
    else
        A= loadmat([heartpath '_thorax.aedl']);
    end
end
% GEOM.AMAtor = 40*loadmat([heartpath '_thorax.vedl']);

if ~isempty(strfind(GEOM.type,'atria'))
    GEOM.AMA=zeromean(32*A(1:min(size(A,1),length(GEOM.TVER)),:));
    ...
    %1_lcav.tri 3 -g %1_rcav.tri 3 -g %1_rlung.tri 0.2 -g %1_llung.tri 0.2 -g %1_atria.tri     1
    ...
    if size(A,1) > length(GEOM.TVER)
        n = length(GEOM.TVER);
        GEOM.AMALCAV = zeromean(40*A(n+1: n + length(GEOM.LVER),:));
        n = n + length(GEOM.LVER);
        GEOM.AMARCAV = zeromean(40*A(n+1: n + length(GEOM.RVER),:));
        
        
        n = n + length(GEOM.RVER);
        GEOM.AMALL = zeromean(40*A(n+1: n + length(GEOM.LLVER),:));
        n = n + length(GEOM.LLVER);
        GEOM.AMARL= zeromean(40*A(n+1: n + length(GEOM.RLVER),:));
        n = n + length(GEOM.RLVER);
...
if 0 && ~isempty(strfind('_ventricles',type)) && isfield(GEOM,'typ')
