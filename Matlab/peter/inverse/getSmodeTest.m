function [S,S1]=getSmodeTest(T,dep,rep,SPECS,mode,varargin)
...
if mode>=4
    ...
    if SPECS.useCumsum
        ...
        apd = rep - dep ;
        slope =0.2;
        initialSlope = slope ./ apd;
        repslope = SPECS.repslope ./ 1.2;

        P = 1 - bsxfun(@times,TDEP,initialSlope);
%         P(P < 0.0) = 0.0;
%         P(P>1) = 1;
             
%         Y = (1./(1 + exp( SPECS.plateauslope.* TREP )))./(1 + exp(SPECS.repslope  .*TREP)); 
        Y = (1./(1 + exp(repslope  .*TREP))); 
%         Y = 1- bsxfun(@times,cumsum(Y,2),1./sum(Y,2));
        
        
        Y1 = (SPECS.initialSlope + 1./(1 + exp( SPECS.plateauslope.* TREP )))./(1 + exp(SPECS.repslope .*TREP)); 
        Y1 = 1 - bsxfun(@times,cumsum(Y1,2),1./sum(Y1,2));
...
%         S=bsxfun(@rdivide,S,max(S,[],2)); % oostep1: 20121015 much faster
        S1=bsxfun(@rdivide,S1,max(S1,[],2)); % oostep1: 20121015 much faster
    end
    if length(dep)==1
    clf
    plot(S1,'k')
    ...
    plot(S.*P,'r')
    plot(P,'g')
    legend('current cumsum','two logistic functions (dep and rep)','with apd slope line','apd line')
    title( [' apd ' num2str(apd) '   slope -' num2str(slope)])
