function varargout = readCineEcgResults2(varargin)
...
for i=1:length(CASEDATA)    
    ...
    if isfield(CASEDATA{i},'selectedVentricularBeats')
        vkey = 'selectedVentricularBeats';
    else
        vkey = 'medianvresults';
    end
    if isfield(CASEDATA{i},vkey)
  
        if isfield(CASEDATA{i},'selectedVentricularBeats') %%isfield(CASEDATA{i}.selectedVentricularBeats,'MEANTSIDIAGNOSTIC')
    ...
    if isfield(CASEDATA{i},'selectedAtrialBeats')
        akey = 'selectedAtrialBeats';
    else
        akey = 'medianaresults';
    end
    
    if isfield(CASEDATA{i},akey)
        if isfield(CASEDATA{i},'selectedAtrialBeats')
...
elseif isfield( SPECS, 'onsetP') 
    SPECS.PwaveDuration         = SPECS.endP - SPECS.onsetP;
end

SPECS.beatCineECG                                          = read3DVertices(BEAT.beatCineECG);
SPECS.beatVcg                                              = read3DVertices(BEAT.beatVcg);
if isfield(BEAT, 'beatCineECGHeart'),SPECS.beatCineECGHeart= read3DVertices(BEAT.beatCineECGHeart);end
if isfield(BEAT, 'beatIsoCineECGHeart'),SPECS.beatIsoCineECGHeart= read3DVertices(BEAT.beatIsoCineECGHeart);end
if isfield(BEAT, 'medianPathECGdeviation'),
    SPECS.medianPathECGdeviation= read3DVertices(BEAT.medianPathECGdeviation);
    if isempty(SPECS.medianPathECGdeviation)
        stop=1;
    end
end
