...
function varargout = selectBSMBeats(varargin)
...
ITRI=[];
elecs=[];
LAY = [1 12 0;[ ones(12,1) (1:12)' (1:12)']];
filename=[ ];
usemean = 0;
zoomfull = 1;
funscale = 0.5;
...
while pp<=length(varargin)
    if ischar(varargin{pp})
        ...
        switch key
            ...
            case 'funscal'
                funscale = varargin{pp+1};pp=pp+2;
            case 'lay'
...
% end
if sampT ~= 1/1000
    BSM = resampleAvo(BSM,1,size(BSM,2),size(BSM,2)*sampT*1000);   
end

waitfor( selectBSMbeat(BSM,'lay',LAY,'sampt',1/1000,'zoomfull',zoomfull,'funscal',funscale ) );
    ...
    if usemean && sum(DATA.SELBEATS) == 0
        ...
        for i=1:length(DATA.SELBEATS)
            if DATA.SELBEATS(i)
                ...
                if size(BSMcorrect,1)==12
                    ecg = BSMcorrect(4:end,:);
                    ecg(1:3,:) = ecg(1:3,:) / 1.5;
                    BSMcorrectAll = T * ecg;                    
                else
                    BSMcorrectAll = T * BSMcorrect;
                end
                BSMOUT{k} = BSMcorrect;
                ...
                if ~isempty(filename)
                    ...
                    savemat([filename '_beat' num2str(i) '.allecg'], BSMcorrectAll);
                    saveasci([filename '_beat' num2str(i) '.remove'], DATA.remove);
                end
                k = k+1;
            end
        end    
...
%     figure(1091);clf;
    for i=1:length(DATA.SELBEATS)
        if DATA.SELBEATS(i)
            BSMcorrect = DATA.BSMOUT(:,DATA.BEATS(i):DATA.BEATS(i+1));
%             figure(1091);sigplot_p(BSMcorrect,'lay',LAY);
            BSMOUT{k} = BSMcorrect;
            TIMES{k}.t0 = DATA.BEATS(i);
            TIMES{k}.t1 = DATA.BEATS(i+1);
            if ~isempty(filename)
