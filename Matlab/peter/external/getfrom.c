             		...
             		int nrhs, const mxArray *prhs[] )
{
  /* Declare variables. */  
	...
	if (nrhs ==4)
	{
		...
		if( ( mxGetM(prhs[1])==npnt && mxGetN(prhs[1])==1 ) ||
            ( mxGetM(prhs[1])==1    && mxGetN(prhs[1])==npnt ) )
        {
            dep = mxGetPr(prhs[1]);
        }
        else
        {
            mexPrintf("it is %d x %d",mxGetM(prhs[1]),mxGetN(prhs[1]));
			mexErrMsgTxt("function values must be [npnt x 1]");
        }
        dep = mxGetPr(prhs[2]);
    ...
    plhs[0]=mxCreateNumericMatrix(npnt,1,mxUINT32_CLASS,mxREAL);
    frompnt= mxGetPr(plhs[0]);
    ...
    frompnt[startPnt]=startPnt;
    mexPrintf("startpnt %d \n",startPnt );
