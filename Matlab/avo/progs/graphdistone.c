...
void init_getdistmat()
{
	...
	for ( i=0;i<npnt;i++)
    {
	 	...
	 	for (j = i+1;j<npnt;j++)
        {
    		if (ADJ[inpnt+j] > 0)
			{
	 		 	rib[k][0]=i;
				rib[k][1]=j;
				maxlength = maxlength + ADJ[inpnt+j];
				k++;
			}
        }
    }
    ...
    mexPrintf("init_getdistmat rib %f\n",maxlength);
}
...
void CalculateDists(int start)
{
    ...
    mexPrintf("init_getdistmat\n");	
    init_getdistmat();
    mexPrintf("post init_getdistmat\n");
	for (i=0; i<npnt;i++)
		fixed[i]=0;
	nfix=1;
	DIST[start]=0;
...
void CalculateDistsAndPaths(int start)
{
	...
	while (nfix<npnt)
	{
		...
		if (n>maxNodes)
        {
			mexErrMsgTxt("Too many nodes (>100000) with the same length.");
        }
		for (i=0;i<n;i++)
		{
			if(fixed[rib[fixrib[i]][1]] &&
