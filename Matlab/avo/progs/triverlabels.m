...
% A. van Oosterom; 2005-04-06; method based on peeling the surface 

%function [VER,ITRI]=triverlabels(VER,ITRI,node); 
clear
%[VER,ITRI]=icosasub(2);
%[VER,ITRI]=loadtri('../../broddel/uprofile.tri');
[VER,ITRI]=loadtri('../album/pp20/atriavo_1.tri');
...
tridist=mean(verdist(ITRI),2);

   
TRISPECS=[(1:ntri)' ITRI tridist];
...
for k=1:kmax,
    ITRI=TRISPECS(TRISPECS(:,5)>k-1&TRISPECS(:,5)<k,2:4)
    clf
    triplot
    pause 
end
