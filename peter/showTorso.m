...
if isempty(posi)
	...
	set(gcf,'Position',[52   121   881   596]);
	posi=[0 0 1 1];
else
	if doclf,clf; else hold on; end
% 	posi;
end
if posi(3)>.5 && dobar
...
%%
if dobar
	axes('Position',[posi(1)+0.01 posi(2)+0.01 0.45*posi(3) 0.98*posi(4)]);
else
	axes('Position',[posi(1)+0.01 posi(2)+0.01 0.5*posi(3) 0.95*posi(4)]);
end
patch('Faces',ITRI,'Vertices',VER,...
	  ...
	  'edgecolor',edges,'FaceAlpha',1,'buttondownFcn','selectnode');
axis off equal; 
caxis([mindep,maxdep]);
view(myview);
...
if posi(3)>.5
	...
	if isolines,contourlines(VER,ITRI,dep,'delta',sublines,'labels',0,'odd',0);  end
else
	if isolines,contourlines(VER,ITRI,dep,'delta',20,'labels',0,'odd',0);  end
end
if ~isempty(endoVER)
    DrawLines(VER,ITRI,endoVER);
end
text(mean(VER(:,1))-40,mean(VER(:,2))-20,max(VER(:,3)+30),'front')

% back
if dobar
	axes('Position',[posi(1)+0.45 posi(2)+0.01 0.45*posi(3) 0.98*posi(4)]);
else
	axes('Position',[posi(1)+0.5 posi(2)+0.01 0.5*posi(3) 0.95*posi(4)]);
end
patch('Faces',ITRI,'Vertices',VER,...
	  'FaceLighting','phong','BackFaceLighting','reverselit','AmbientStrength',0.7,...
	  'FaceVertexCData',dep,'FaceColor','interp',...
...
text(mean(VER(:,1))-40,mean(VER(:,2))+10,max(VER(:,3)+30),'back')
