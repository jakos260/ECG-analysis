function struct = XML2Struct(filename) 
global i_xml2struct 
...
i_xml2struct.struct = xmlread(txt(xml0:end)); 
if nargout==0 
   ...
   struct = i_xml2struct.struct; 
end 



%----------------------- 
function [struct,last] = xmlread(txt) 
global i_xml2struct 
struct = []; 
first = 1; 
txtlen = length(txt); 
...
while (1) 
   ...
   [item,last] = xmlread(txt(first:last)); 
   if ischar(item) 
      item = strrep(item,'&lt;','<'); 
      item = strrep(item,'&gt;','>'); 
   ...
   last = last+first-1; 
   if isfield(struct,tag) 
      tmp = getfield(struct,tag); 
      if ischar(tmp) 
         l = size(tmp,1); 
      else 
         l = length(tmp); 
      ...
      if (l==1) 
         struct = setfield(struct,tag,{tmp,item}); 
      else 
         eval(sprintf('struct.%s{%d}=item;',tag,l+1)); 
      end 
      ...
      struct = setfield(struct,tag,item); 
   end 
