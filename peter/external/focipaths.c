...
//         }
        ...
        foci  = mxCalloc(npnt,sizeof(double)); foci || outofmem();
        for (i=0;i < nFoci;i++)
        {
            foci[i] = fociIn[i]-1;
            if ( foci[i] >= npnt ||
        ...
        while ( nfix < npnt )
        {
            ...
            if ( nfix == nprevfix )
            {
                // these extra runs are required because the non fixed foci 
                // can cause the number of fixed nodes to be equal to the 
                // one of the previous run (distance[focus] < length)
                if ( testFix == 0 )
                {
