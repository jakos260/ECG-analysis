function [X,Y,Z,C]=plotcontourlines(varargin)% VER,ITRI,VALS,sym)
...
maxi=max(VALS(~isinf(VALS)));
mini=min(VALS(:));
maxi=120;
mini=0;
if maxi,
    maxim=real(ceil(log10(maxi))-1);		 
else
    maxim=-10000; 
end
if mini, 
    minim=real(ceil(log10(abs(mini)))-1);
else
    minim=-10000; 
end
...
%   x=contour(cnt_indx).intersect1;
%   patch('Xdata',x(:,1),'Ydata',x(:,2),'Zdata',x(:,3),'edgecolor','k');
...
% contour3([intersect1(:,1),intersect1(:,2),cntlevel(:)])
...
colormap(jet)
mymap=colormap;
scale2Color= length(mymap)/(maxi - mini);
for i=1:length(cntlevel)
    mycolor  = round(scale2Color*cntlevel(i));
    mycolor = min(length(mymap),max(1,mycolor));
	if cntlevel(i)>0
	    h1 = line(Y(:,i), X(:,i), Z(:,i),'color',mymap(mycolor,:),'userdata',cntlevel(i),'linestyle','-','linewidth' ,3);
	elseif cntlevel(i)<0
		h1 = line(X(:,i), Y(:,i), Z(:,i),'color',mycolor,'userdata',cntlevel(i),'linestyle','--');
	else
		h1 = line(X(:,i), Y(:,i), Z(:,i),'color',mycolor,'userdata',cntlevel(i),'linestyle',':','linewidth',1);
	end
    hc = [hc; h1];
end
caxis([mini maxi])
...
% 	ib=cntlevel(i);	
	...
	while i<length(cntlevel) %&& cntlevel(i)~=cntlevel(end)
		...
		if length(a) >10 && ( deltal>=deltalab)%&& cntlevel(i)~=cntlevel(end)%|| i==length(cntlevel)
			...
			c=mean(b); c=c./norm(c);
			c=get(gca,'CameraTarget')-get(gca,'CameraPosition');c=-1*c/norm(c);
			...
			text(x,y,z,txt,'HorizontalAlignment','center','EraseMode','normal','color','k','fontname','verdana','fontsize',6);
			a=find(cntlevel==cntlevel(i));
			i=a(end)+1;
