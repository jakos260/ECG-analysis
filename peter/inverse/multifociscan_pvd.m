function [bestfoci,bestdep,outp]=multifociscan_ucla(varargin)
...
SCAN.L          = surflapl(GEOM.VER,GEOM.ITRI,0);
...
while  nrClust < 1 ||  max(bestdep) > SCAN.qrsduration + delta
...
for inode=1:length(GEOM.VER)
    ...
    PSIA =lowpassma(SCAN.AMA*getSmode(ones(length(GEOM.VER),1) * (1:maxt),dep,rep, GEOM.SPECS, SCAN.scanmode,GEOM),SCAN.lpass);
    if isempty( initdep )
        PSIA2 =lowpassma(SCAN.AMA*getSmode(ones(length(GEOM.VER),1) * (1:SCAN.qrsduration),dep,rep, GEOM.SPECS, SCAN.scanmode,GEOM),SCAN.lpass);
...
if 0 && isempty(initdep)
    ...
    %'KORS et al.'
    %[V KI_9=[ -0.5267    0.1634   -0.2867   -0.1300    0.0500   -0.0100    0.1400    0.0600    0.5400;   % Vx
...
%    
    ...
    for ij=1:length(ii)
        ...
        dip = estimateDipole(SCAN,GEOM.BSM(:,GEOM.SPECS.onsetqrs:GEOM.SPECS.onsetqrs+GEOM.SPECS.time_Jpoint),SCAN.useleads,dipLoc);
