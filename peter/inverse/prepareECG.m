...
function SPECS = prepareECG(BSM,LAY,neigh,varargin)
...
SPECS = estimatedFromTdom(SPECS,neigh,funtype);
...
drrms=rms(diffrows(ECG));
drrms = 2*drrms/min(1,max(drrms))*LAY(1,2);
nt = length(rrms);

t=(1:nt)/nt;
% plot(t,lowpassma(rrms,10),'r')
plot(t,rrms,'r'); % oostep1: not nice with pacing spikes
plot(t,drrms,'m'); % oostep1: not nice with pacing spikes
if selectFiducial && doAtria
    ...
    disp('end Pwave?')
    [x,~]=ginput(1);
    SPECS.endP=max(1,round(x/t(nt)*nt));
...
% tdom = zeros(1,nt);
ntt = size(PSI,2);
tdom = ones(1,ntt)*(PSI'*PSI);

if any(tdom(1:SPECS.time_Jpoint)<0)    
    tdomB = baselinecor(tdom,SPECS.time_Jpoint-20,length(tdom));
    tdom = tdomB;
%     if any(tdomB(1:SPECS.time_Jpoint)<0)    
    negTdom=find(tdom<=0);
    negTdom(negTdom>SPECS.time_apexT)=[];
    negTdom = max(negTdom(end)+10,SPECS.time_Jpoint);    
    [tdom,ttm]=getTdom(PSI,negTdom );
else
    [tdom,ttm]=getTdom(PSI,SPECS.time_Jpoint);
end
tdom(1:SPECS.time_Jpoint) = max(0,tdom(1:SPECS.time_Jpoint));
if tdom(1)<0
    tdom = baselinecor(tdom);
end
tdom = tdom * rrms(SPECS.time_apexT)/max(tdom(SPECS.time_Jpoint:length(tdom)));
...
% exponential fit of the part of the dominant T wave under the QRS
fact = 0.1;
tau=-jpoint/log(fact );
j=1:jpoint;
tdom(j)=fact *y1*exp(j/tau);
tdom=tdom/sum(tdom);
    


%% ------------------------------------------------------------------------
function SPECS = estimatedFromTdom(SPECS,neigh,funtype)
...
paramsCumsum(5) = paramsCumsum(5) - SPECS.ttm;
...
if SPECS.useCumsum
    ...
    S1 = getSmode(t, dep,    rep - 100, SPECS,4,neigh);
    S2 = getSmode(t, dep+100,rep - 100, SPECS,4,neigh);
else
    S  = getSmode(t, dep,    rep,       SPECS,3,neigh);
    S1 = getSmode(t, dep,    rep - 100, SPECS,3,neigh);
    S2 = getSmode(t, dep+100,rep - 100, SPECS,3,neigh);
end
