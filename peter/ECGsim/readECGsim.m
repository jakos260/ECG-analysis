...
if exist([modeldir  'atria.tri'],'file')
    ...
    if length(DATA.ATRIA.lead12 ) <= 1
        ...
        DATA.ATRIA.VENTR212lead = DATA.ATRIA.lead12( [ 1:3 10:12 4:9],:);
    end
end
...
if length(DATA.VENTR.VENTR212lead) <= 1
    DATA.VENTR.lead12 = loadmat([modeldir  'ventricles2standard_12.mat']);    
    DATA.VENTR.lead12 = DATA.VENTR.lead12( [ 1:3 10:12 4:9],:);
end
...
for i=1:length(ecgs)
    ...
    a=strrep(a,' ','_');
    if contains(a,'standard_12')
        eval([a 'elec = loadmat(''' [ecgdir ecgs(i).name(1:end-6)] 'elec' ''');']);
        eval(['A = ' a 'elec;'])
        if size(A,1)==9 && abs(A(end,4) - A(end-1,4)) > 200 % change
            eval([a ' = loadlead12(''' [ecgdir ecgs(i).name] ''');']);
            A = A([7:end 1:6],:);
            eval([a 'elec = A;'])
        end        
    else
    ...
    isE = eval(['isempty(' a 'elec);']); 
    if isE
        eval([a 'elec = DATA.GEOM.thorax.VER;'])
    else
...
%% 
function [M, extraresult]=loadlead12(name)
[M, extraresult]=loadmat(name);
M = M([ 1:3 10:12 4:9],:);
