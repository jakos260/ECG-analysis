...
void showHelp(void)
{
    ...
    mexPrintf("S = getSp(mode, maxt, ADJ, dep, rep, ,mode,repparams),  \n");
    ...
    mexPrintf("		 4) S NOT using the cumsum (pvd), requires a) initial repslope, b) plateau slope and c)rep slope\n");
	mexPrintf("maxt: maximum time, is the number of columns of S \n");    
	mexPrintf("ADJ : adjacency matrix with graphdist(ITRI) \n");
    mexPrintf("dep : the depolarization time (size of dep needs to be equal to the size of ADJ)\n");
	mexPrintf("rep : the depolarization time (size of dep needs to be equal to the size of ADJ)\n");
	mexPrintf("repparams: This depends on the mode \n");
	mexPrintf("		  mode == 2	 plateau slope and rep slope (ECGSIM version <= 3.x)\n");
	mexPrintf("		  mode == 3	 initial repslope, b) plateau slope and c)rep slope \n");
    mexPrintf("		  mode == 4	 initial repslope, b) plateau slope and c)rep slope \n");
}
...
{    
    int i,j, ti;
    double maxNeighT = mDep[ir];
    double minNeighT = mDep[ir];
    double deltaT;
    j = ir * mNpnt;
	for (i=0; i < mNpnt; i++ )
	{		
    ...
    deltaT = maxNeighT - minNeighT;       
    for ( ti = 0; ti <= minNeighT && ti < mMaxt; ti++ )
    {
        rowS[ti] = 0.0;
    }    
    if ( deltaT > 1.0)
    {
        double val = 1.0 - fmod(minNeighT,1.0);
        for ( ; val < deltaT && ti < mMaxt; ti++ )
        {
            rowS[ti] = val / (deltaT);  
            val = val + 1.0;
        }
        ...
        if ( mMode > 1 )
        {
            ...
            if ( mSourceVersion == 1 )
            {
                for ( t = 0; t < mMaxt; t++, tdep += 1.0, trep += 1.0 )
                {
                    ...
                    // when no dispersion for the neighbor is availble the TMP makes a step which is faster than the sigmoid
                    if ( rowS[t] > maxAmpl )
                    {
                        maxAmpl = rowS[t];
                    } 
            ...
            else if ( mSourceVersion == 2 )
            {
                ...
                for ( t = 0; t < mMaxt; t++, tdep += 1.0, trep += 1.0 )
                {
                    vali =(mRepParams[0] +  1 / ( 1 + exp( mRepParams[1] * trep ) ) ) *
                                          ( 1 / ( 1 + exp( mRepParams[2] * trep ) ) );
                    rowS[t] *= vali;
                    // The  "anti hakkel" can make the rep times shift due to the scaling of the max TMP 
                    // therefore the standard upstroke velocity is used to scale
                    vali *= 1.0 /( 1.0 +  exp(-2.0 * tdep ) );
                    if ( vali > maxAmpl )
                    {
                        maxAmpl = vali;
            ...
            else // mSourceVersion == 0 cumsum
            {
                sumY = 0;
                trep    = -mRep[ir];

                for ( t = 0; t < mMaxt; t++, trep += 1.0 )
                {
                    sumY +=   (mRepParams[0] +  1 / ( 1 + exp( mRepParams[1] * trep ) ) ) *
                                              ( 1 / ( 1 + exp( mRepParams[2] * trep ) ) );

                    y[t] = sumY; // cumsum
                }
                tdep    = -mDep[ir];
                for (t=0; t < mMaxt; t++, tdep += 1.0  )
                {                    
                    vali = 1 - (y[t]/sumY); // 1 - cumsum
