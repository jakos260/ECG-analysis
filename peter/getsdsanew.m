...
function S=getsdsanew(T,dep,rep,p,VER,ITRI,OBS,mode)
nt=size(T,2);
nn=size(T,1);
...
if mode~=1,
    ...
    verareas=zeros(size(VER,1),size(S,2));
    for k=1:length(OBS)
        OMEGAtri = dsa(VER,ITRI,OBS(k,:),0.1);
        OMEGA = zeros(size(VER,1),1);
        for j=1:size(ITRI,1),
            ij=ITRI(j,:);
            OMEGA(ij)=OMEGA(ij) + OMEGAtri(:,j);
        end
        for i = 1:size(ITRI,1)
            ...
            for t=0:max(dep)+1
                if t < dep(i1)
                    ...
                    if t >= dep(i3)
                        verareas(i1,t+1:end) = verareas(i1,t+1:end) + OMEGAtri(1,i)* Stridep(i,t+1:end);
                        verareas(i2,t+1:end) = verareas(i2,t+1:end) + OMEGAtri(2,i)* Stridep(i,t+1:end);
                        verareas(i3,t+1:end) = verareas(i3,t+1:end) + OMEGAtri(3,i)* Stridep(i,t+1:end);
                        break;
                    else
                        verareas([i2 i3],t+1) = 0;
                        rm = VER(i1,:) + (VER(i2,:) - VER(i1,:)) * min(1.0,max(0,(t-dep(i1))) / (dep(i2)-dep(i1)));
                        rp = VER(i1,:) + (VER(i3,:) - VER(i1,:)) * min(1.0,max(0,(t-dep(i1))) / (dep(i3)-dep(i1)));
                        triarea = dsa([VER(ITRI(i,1),:); rm; rp],[1,2,3],OBS(k,:),0.1);
%                         triarea = norm(cross(rp,rm))/2.0;
                        triarea = triarea * Stridep(i,t);
                        verareas(i1,t+1) = verareas(i1,t+1) + triarea(1);
                        verareas(i2,t+1) = verareas(i2,t+1) + triarea(2);
                        verareas(i3,t+1) = verareas(i3,t+1) + triarea(3);
                    end
                end
            end
        end
    ...
    S = S .* verareas ./ (OMEGA* ones(1,size(T,2)));
