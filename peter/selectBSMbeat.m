...
if length(varargin) < 1
    ...
    sumData = mean(abs(DATA.ORG - ( lowpassma(DATA.ORG, 1 / ( 0.1 * DATA.sampT ) ) )),2);
    % sumData = mean((DATA.ORG),2);
    DATA.remove = sumData > 7 * std(sumData);
%         DATA.remove([51,61,63,34,40])=1 % temp for 059
    if isfield(DATA,'PQint')
        set(handles.PQint,'String',num2str(DATA.PQint));
    else
        pqint = str2double(get(handles.PQint,'String')); % PQ interval in ms   
    ...
    set(handles.sliderSignal,'max',1); 
...
BSMfiltextra = BSM - ( lowpassma(BSM, round(1 / ( 10.0 * DATA.sampT ) ) ) ); % for baseline drift
...
%     qrss(qrss < 300 )=[];
    ...
    if (length(DATA.BEATS) < size(DATA.BSM,2)/2000 || max(diff(DATA.BEATS))>6000) && size(BSMfiltextra,2) > 2000
        ...
        DATA.BEATS = qrss(1) - DATA.PQint;
        for i=2:length(qrss)
            if qrss(i) - qrss(i-1) > 300 % minimal interval between qrss
                DATA.BEATS = [DATA.BEATS; qrss(i) - DATA.PQint];
            end
        end
    end
...
if ~isempty(DATA.BEATS)
    ...
    BASEL=spline(zerotims,DATA.ORG(:,zerotims),1:size(DATA.ORG,2));
    DATA.BSMOUT = DATA.ORG - BASEL;
    DATA.BSM = DATA.ORG - BASEL;
    DATA.BSMfilt    = DATA.BSMfilt - spline(zerotims,DATA.BSMfilt(:,zerotims),1:size(DATA.BSMfilt,2));           
else
    DATA.BSMOUT = DATA.BSM;
end
DATA.RMS     = rms(DATA.BSM);
...
axis(handles.RMSaxes,[rmst(1) rmst(end) minA maxA]);
...
X = abs(DATA.LAY(2:end,1)-x);
Y = abs(DATA.LAY(2:end,2)-y);
signalIndex = find(X==min(X) & Y==min(Y));
signalIndex = DATA.LAY(signalIndex+1,3);
...
if OPTIONS.funscal > 0.5
    OPTIONS.funscal = max(0.01,OPTIONS.funscal - eventdata.VerticalScrollCount/5.0);
else
    OPTIONS.funscal = max(0.01,OPTIONS.funscal - eventdata.VerticalScrollCount/50.0);
end
plotBSM(handles);
...
[OPTIONS.FileName,OPTIONS.PathName,FilterIndex] = uigetfile({'*.mat','*.*'},'Select data file.','/Users/peteroosterhoff/Documents/Werk/STW/Data/Pig08/Biosemi/Export');
