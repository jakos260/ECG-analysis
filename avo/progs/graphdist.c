/*************************** graphdist.c ******************************************************/
/*                                                                                    
GRAPHDIST Version: 2.02
Determines: shortest distances, shortest paths; supports computing adjacency matrix for triangulated surfaces
...
[DIST,PATHS]=graphdist(ADJ) 
    ...
    Returns the distance matrix and (optionally) the prominent routes matrix based on 
             any precomputed (ADJ) adjacency matrix of a weighted or unweighted graph. Graph need not be fully connected
             unconnected nodes signalled in DIST by -1 entries 
     DIST  : the distance matrix. Specifies the distances between all nodes while traveling along edges.
     PATH  : the prominent routes matrix. Each column (k) in PATH contains the number of times the node
...
[ADJ,DIST,PATH]= graphdist(ITRI), 

     Ass in case 3; but ADJ used: UN-weighted adjacency matrix derived from triangle specification ITRI 
 
option 3:                    

[ADJ,DIST,PATH]= graphdist(ITRI,VER,mode)

      As in option 2 but ADJ now a pertaining to WEIGHTED graph.
           
                      - mode 1 (default) : ADJ weighted by edge lengths. 
                      - mode 2           : ADJ based on internodal distantances in 3D space, provided 
                                               that they can 'see' one another in the interior of the triangulated surface. 
                      - mode 3           : As  in mode 2, but now the second order neigbors are also treated as direct neighbors.
                      - mode 4           : As   mode 1, but now the second order neigbors are treated as direct neighbors too.
See also
- getroute, graphdistone 
/*                                                                         
/* 05-03-12  Peter van Dam                                                
/*           Peacs, Arnhem The Netherlands		                                          
/*                                                                         
/***************************************************************************************************/
...
void showHelp(void)
{
    mexPrintf("GRAPHDIST Version: 2.1\n");
    mexPrintf("Determines: shortest distances, shortest paths; supports computing adjacency matrix for triangulated surfaces\n\n");
    ...
    mexPrintf("- Option 1       [DIST,PATH]= graphdist(ADJ)\n");
    mexPrintf("     Returns : the distance matrix and (optionally) the prominent routes matrix based on \n");
    mexPrintf("               any precomputed (ADJ) adjacency matrix of a weighted or unweighted graph. \n");
    mexPrintf("               Graph need not be fully connected unconnected nodes signalled in DIST by -1 entries \n");
    mexPrintf("     DIST    : the distance matrix. Specifies the distances between all nodes while traveling along edges.\n");
    mexPrintf("     PATH    : the prominent routes matrix. Each column (K) in PATH contains the number of times the node\n");
    ...
    mexPrintf("- Option 2       [ADJ,DIST,PATH]= graphdist(ITRI)\n");
    mexPrintf("     As in option 3; but ADJ used: UN-weighted adjacency matrix derived from triangle specification ITRI \n\n");
    mexPrintf("- Option 3       [ADJ,DIST,PATH]= graphdist(ITRI,VER,mode)\n");
    mexPrintf("     As in option 2 but ADJ now a pertaining to WEIGHTED graph.\n");           
    mexPrintf("         - mode 1 (default) : ADJ weighted by edge lengths. \n");
	mexPrintf("         - mode 2           : ADJ based on internodal distantances in 3D space, provided \n");
    mexPrintf("                              that they can 'see' one another in the interior of the triangulated surface.\n"); 
    mexPrintf("         - mode 3           : As  in mode 2, but now the second order neigbors are also treated as direct neighbors.\n");
...
void CalculateDists(void)
{
    ...
    for (start=0;start<npnt;start++)
    {
        ...
        while ( nfix < npnt)
        {
            ...
            {
                break;
                //                mexPrintf("Inconsistent graph (more than 1 part).");
            }
            else if (n > maxNodes)
                mexErrMsgTxt("Too many nodes (>10000) with the same length.");
            else
            {
                for (i=0;i<n;i++)
                {
...
void CalculateDistsAndPaths(void)
{
    ...
    for (start=0; start<npnt;start++)
    {
        ...
        PATH[start+start*npnt] = nStarted;
    }
    mxFree(frompnt);
    mxFree(fixed);
    mxFree(rib);
