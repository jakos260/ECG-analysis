% file triplot_contour.m
%  specify: VER;ITRI;VALS
%  defaults set: column=1,
%                node=1;
%                cmap='avojet.mcm';
%                symcolor=0; symcolor==1 produces colorbar symmetric around zero
%                boxsize= max(norm3d(VER)); may be set externally; 
%                zebra=0; zebra>nzebra produces nzebra stripes (isofunction
%                         lines) zebra<0 produces isofunction lines at abs(zebra)
%                         intervals
%                zebra is overruled by:  stripes=[desired levels]
%                no contour lines are shown of neither zebra nor stripes have been specified 
%                nobar=0;  nobar=1 suppresses colorbar display
%                contourcolor='white'
% run: triplot_hide_uis to temporarily hide the UIs
...
% set defaults%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ~exist('newboxsize','var'),
    boxsize=max(norm3d(VER));
else,
    boxsize=newboxsize;
end

if ~exist('nobar','var') nobar=0                     ; end  % brute force for suppressing colorbar
if ~exist('boxsize','var'), boxsize=max(norm3d(VER)) ; end
if ~exist('contourcolor','var'), contourcolor='white'; end
nosw=0;
funsw=0;
fun=zeros(nver,1);
ndat=1;
...
if ~exist('lsw','var') lsw=1; end
%if ~exist('zebra')  zebra=0; end
if ~exist('node','var')   node=min(unique(ITRI)); end
if ~exist('column','var') column=1;end
if ~exist('symcolor','var'), symcolor=0; end % symcolor==1 produces colorbar symmetric around zero
if ~exist('lsw','var'),lsw=1; end
...
if ~exist('iview','var'), iview=1; end
if ~exist('azim','var'), azim=90; elev=0; end 

if exist('cmap','var')==0, cmap='avojet.mcm'; cmapnow=cmap; end
...
if exist('VALS','var')&& (exist('zebra','var')|| exist('stripes','var') ),
    [X,Y,Z,C]=plotcontourlines(VER,ITRI,VALS)
end
...
% show nodes
if ~exist('nodesval','var'), nodeswval=0;, end
...
set(ui17,'foreground','r','fontsize',12);
...
if exist('VALS','var') & funsw==1, set(ui15,'vis','off');end
if ~exist('VALS','var')|isempty(VALS)==1, set([ui16;ui17],'vis','off');
else,    
    set([ui16;ui17],'vis','on');
end   
